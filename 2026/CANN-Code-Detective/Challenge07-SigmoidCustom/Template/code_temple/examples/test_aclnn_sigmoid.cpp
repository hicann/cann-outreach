...
int main()
{
    ...
    std::vector<float> yHostData(16384, 0);
    ret = CreateAclTensor(yHostData, yShape, &yDeviceAddr, aclDataType::ACL_FLOAT, &y);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    

    // 调用 aclnnSigmoidGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnSigmoidGetWorkspaceSize(x, y, &workspaceSize, &executor);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnSigmoidGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnSigmoid 第二段接口
    ret = aclnnSigmoid(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnSigmoid failed. ERROR: %d\n", ret); return ret);
