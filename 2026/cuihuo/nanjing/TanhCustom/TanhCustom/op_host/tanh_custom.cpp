...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    for (uint32_t i = 0; i < context->GetInputShape(0)->GetStorageShape().GetDimNum(); i++) {
        totalLength *= context->GetInputShape(0)->GetStorageShape().GetDim(i);
    }
    context->SetBlockDim(BLOCK_DIM);
    tiling.totalLength = totalLength;
    tiling.tileNum = TILE_NUM;
    tiling.tileLength = (totalLength + TILE_NUM - 1) / TILE_NUM;
    tiling.lastTileLength = totalLength - (TILE_NUM - 1) * tiling.tileLength;
    tiling.ubBufferSize = 256 * 1024;
