...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Exp(tmpTensor0, xLocal, this->tileLength);          // exp(x)
        AscendC::Muls(tmpTensor1, xLocal, DTYPE_X(-1), this->tileLength);  // -x
        AscendC::Exp(tmpTensor1, tmpTensor1, this->tileLength);      // exp(-x)
        AscendC::Sub(tmpTensor2, tmpTensor0, tmpTensor1, this->tileLength);  // exp(x) - exp(-x)
        AscendC::Add(tmpTensor0, tmpTensor0, tmpTensor1, this->tileLength);  // exp(x) + exp(-x)
        AscendC::Div(yLocal, tmpTensor2, tmpTensor0, this->tileLength);      // (exp(x) - exp(-x)) / (exp(x) + exp(-x))
