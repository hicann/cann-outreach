...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    // ---- 核内切分（Tile Tiling） ----
    // UB 分配：
    //   inQueueX  × BUFFER_NUM(2) × tileLength × dtypeSize
    //   outQueueY × BUFFER_NUM(2) × tileLength × dtypeSize
    //   tmpBuf0   × 1             × tileLength × 4(float)
    //   tmpBuf1   × 1             × tileLength × 4(float)
    //   tmpBuf2   × 1             × tileLength × 4(float)
    // 合计：bufferCoefficient = 2*2*dtypeSize + 3*4
    uint32_t bufferCoefficient = 2u * 2u * dtypeSize + 3u * 4u;
    uint32_t maxTileElements   = static_cast<uint32_t>(ubSize) / bufferCoefficient;
    // 对齐到 32B 边界
