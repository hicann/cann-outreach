...
private:
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        uint32_t copyLength = (offset + this->tileLength > this->blockLength)
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<uint8_t> tmpBuffer = tmpBuf0.AllocTensor<uint8_t>();

        uint32_t calLength = (progress * this->tileLength + this->tileLength > this->blockLength)
                                 ? (this->blockLength - progress * this->tileLength)
                                 : this->tileLength;
        AscendC::Tanh(yLocal, xLocal, tmpBuffer, calLength);

        outQueueY.EnQue<DTYPE_Y>(yLocal);
        inQueueX.FreeTensor(xLocal);
        tmpBuf0.FreeTensor(tmpBuffer);
    }
