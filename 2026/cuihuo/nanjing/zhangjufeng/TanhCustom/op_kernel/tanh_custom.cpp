...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        pipe.InitBuffer(tmpBuf0, this->tileLength * sizeof(DTYPE_X)); // exp(x)
        pipe.InitBuffer(tmpBuf1, this->tileLength * sizeof(DTYPE_X)); // exp(-x)
        pipe.InitBuffer(tmpBuf2, this->tileLength * sizeof(DTYPE_X)); // denominator
    }
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        LocalTensor<DTYPE_X> denom = tmpBuf2.Get<DTYPE_X>();

        // exp(x)
        Exp(expX, xLocal, this->tileLength);
        // exp(-x) = exp(x * (-1))
        Muls(expNegX, xLocal, (half)-1.0, this->tileLength);
        Exp(expNegX, expNegX, this->tileLength);

        // numerator = exp(x) - exp(-x)
        Sub(yLocal, expX, expNegX, this->tileLength);
        // denominator = exp(x) + exp(-x)
        Add(denom, expX, expNegX, this->tileLength);
        // y = numerator / denominator
        Div(yLocal, yLocal, denom, this->tileLength);
