...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        ASSERT(this->blockLength % (this->tileNum * BUFFER_NUM) == 0);
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        // Step 2: tmp1 = x * (-1.0) = -x
        AscendC::Muls(tmp1, xLocal, (half)-1.0, this->tileLength);
        // Step 3: tmp2 = exp(-x)
        AscendC::Exp(tmp2, tmp1, this->tileLength);
        // Step 4: tmp1 = exp(x) - exp(-x)
        AscendC::Sub(tmp1, tmp0, tmp2, this->tileLength);
        // Step 5: tmp0 = exp(x) + exp(-x)
        AscendC::Add(tmp0, tmp0, tmp2, this->tileLength);
        // Step 6: y = numerator / denominator
