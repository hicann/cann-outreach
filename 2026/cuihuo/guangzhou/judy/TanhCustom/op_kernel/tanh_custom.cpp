...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Exp(tmp0, xLocal, this->tileLength);                          // tmp0 = exp(x)
        AscendC::Muls(tmp1, xLocal, (half)(-1.0), this->tileLength);           // tmp1 = -x
        AscendC::Exp(tmp1, tmp1, this->tileLength);                            // tmp1 = exp(-x)
        AscendC::Sub(yLocal, tmp0, tmp1, this->tileLength);                    // yLocal = exp(x) - exp(-x)
        AscendC::Add(tmp2, tmp0, tmp1, this->tileLength);                      // tmp2 = exp(x) + exp(-x)
        AscendC::Div(yLocal, yLocal, tmp2, this->tileLength);                  // yLocal = 分子 / 分母
