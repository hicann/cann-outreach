...
// ==========补齐缺失的 PrepareModel 实现==========
Result SampleRes50ImagenetClassification::PrepareModel(const char *modelPath)
{
    INFO_LOG("PrepareModel begin, model path: %s", modelPath);
    uint32_t modelId = 0;
    aclError ret = aclmdlLoadFromFile(modelPath, &modelId);
    if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclmdlLoadFromFile failed, ret = %d", static_cast<int32_t>(ret));
        return FAILED;
    }
    modelId_ = modelId;
    ...
    if (modelDesc_ == nullptr) {
        ERROR_LOG("create AclModelDesc failed");
        return FAILED;
    }

    modelWork_ = std::make_shared<AclModelWork>(modelId_);
    modelWeight_ = std::make_shared<AclModelWeight>(modelId_);

    INFO_LOG("PrepareModel success, modelId: %u", modelId_);
    return SUCCESS;
}
...
void SampleRes50ImagenetClassification::OutputModelResult() {
    for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
        ...
        aclFloat16 *outDataFp16 = nullptr;
        ...
        if (!g_isDevice) {
            ...
            // copy model output data from device to host
            ret = aclrtMemcpy(outHostData, len, outputBuffer, len, ACL_MEMCPY_DEVICE_TO_HOST);
            if (ret != ACL_SUCCESS) {
                ERROR_LOG("aclrtMemcpy failed, errorCode[%d]", static_cast<int32_t>(ret));
                (void)aclrtFreeHost(outHostData);
                return;
            }
            // host侧：主机内存指针强转为aclFloat16*
            outDataFp16 = static_cast<aclFloat16 *>(outHostData);
        } else {
            // device侧：直接拿device侧buffer，类型aclFloat16*
            outDataFp16 = static_cast<aclFloat16 *>(outputBuffer);
        }

        // map存float置信度，降序排序
        map<float, unsigned int, greater<float>> resultMap;
        // 元素个数：总字节数 / aclFloat16占2字节
        unsigned int elemCount = len / sizeof(aclFloat16);
        for (unsigned int j = 0; j < elemCount; ++j) {
            // aclFloat16 转 float
            float score = aclFloat16ToFloat(*outDataFp16);
            resultMap[score] = j;
            outDataFp16++;
        }
