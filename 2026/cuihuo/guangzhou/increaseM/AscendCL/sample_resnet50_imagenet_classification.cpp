...
#include "acl/acl_base_rt.h"  // aclFloat16 & aclFloat16ToFloat (FP16 output)
...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outData = nullptr;  // FP16 output: use aclFloat16* to receive
    if (!g_isDevice) {
    ...
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      resultMap[aclFloat16ToFloat(outData[j])] = j;  // FP16 -> float conversion
    }
