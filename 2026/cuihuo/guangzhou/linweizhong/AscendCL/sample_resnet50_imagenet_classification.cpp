...
// FP16 output support: aclFloat16 type & aclFloat16ToFloat conversion API
...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // Model output data type is FP16 (--output_type=FP16 in ATC), parse as aclFloat16 (half)
    ...
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      // Convert FP16 (half) to FP32 before ranking/sorting/printing
