/**
<C2><A0>* Copyright (c) 2025 Huawei Technologies Co., Ltd.
<C2><A0>* This program is free software, you can redistribute it and/or modify it under the terms and conditions of
<C2><A0>* CANN Open Software License Agreement Version 2.0 (the "License").
<C2><A0>* Please refer to the License for details. You may not use this file except in compliance with the License.
<C2><A0>* THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
<C2><A0>* INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
<C2><A0>* See LICENSE in the root of the software repository for the full text of the License.
 */
...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(data);
    }
    map<float, unsigned int, greater<float> > resultMap;
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
            *outData = aclFloat16ToFloat(*outData);
            resultMap[*outData] = j;
      outData++;
    }
