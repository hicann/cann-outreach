...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outData = static_cast<aclFloat16*>(outHostData);  // 转换为 FP16 指针
    } else {
      outData = static_cast<aclFloat16*>(data);         // 设备模式，直接转换
    }

    // 使用 map 存储浮点置信度和索引（降序排列）
    map<float, unsigned int, greater<float> > resultMap;
    uint32_t elemCount = len / sizeof(aclFloat16);      // 改为 FP16 元素个数
    for (unsigned int j = 0; j < elemCount; ++j) {
      float val = aclFloat16ToFloat(outData[j]);        // 转换为 float
      resultMap[val] = j;
    }
