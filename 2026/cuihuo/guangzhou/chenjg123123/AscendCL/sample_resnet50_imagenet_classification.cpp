...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // insert start
    uint32_t num = len / sizeof(aclFloat16);
    aclFloat16 *fp16Data = reinterpret_cast<aclFloat16 *>(outData);
    std::vector<float> floatOut(num);
    for (uint32_t j = 0; j < num; ++j) {
        floatOut[j] = aclFloat16ToFloat(fp16Data[j]);
    }
    outData = floatOut.data();
    len = num * sizeof(float);
    // INSERT END
