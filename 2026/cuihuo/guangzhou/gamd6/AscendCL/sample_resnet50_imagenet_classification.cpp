...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // [MODIFIED] FP32 -> FP16: pointer type changed to aclFloat16*
    ...
    // [MODIFIED] Element count now based on sizeof(aclFloat16) (2 bytes per element)
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      // [MODIFIED] Convert FP16 to FP32 before inserting into map
