...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        // tmp0: e^x；tmp1: e^(-x)（后复用为分母）；tmp2: -x（后复用为分子）
        ...
        AscendC::Exp(tmp0, xLocal, this->tileLength);                  // tmp0 = e^x
        AscendC::Muls(tmp2, xLocal, (DTYPE_X)(-1.0), this->tileLength); // tmp2 = -x
        AscendC::Exp(tmp1, tmp2, this->tileLength);                    // tmp1 = e^(-x)
        AscendC::Sub(tmp2, tmp0, tmp1, this->tileLength);              // tmp2 = e^x - e^(-x)  分子
        AscendC::Add(tmp1, tmp0, tmp1, this->tileLength);              // tmp1 = e^x + e^(-x)  分母
        AscendC::Div(yLocal, tmp2, tmp1, this->tileLength);            // yLocal = 分子 / 分母
