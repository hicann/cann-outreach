...
{
  size_t bufferNum =
      aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet());

  for (size_t i = 0; i < bufferNum; ++i) {
    aclDataBuffer *dataBuffer =
        aclmdlGetDatasetBuffer(modelOutput_->GetDataSet(), i);
    ...
    void *outputData = deviceData;

    if (!g_isDevice) {
      aclError ret =
          aclrtMallocHost(&hostData, len);

      if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclrtMallocHost failed, len[%u], errorCode[%d]",
      ...
      if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclrtMemcpy failed, errorCode[%d]",
                  static_cast<int32_t>(ret));
        (void)aclrtFreeHost(hostData);
        return;
      }
    ...
    aclDataType outputType =
        aclmdlGetOutputDataType(
            modelDesc_->GetModelDesc(), i);

    INFO_LOG("output type[%d], output size[%u]",
             static_cast<int32_t>(outputType), len);

    std::multimap<float,
                  unsigned int,
                  std::greater<float>> resultMap;

    if (outputType == ACL_FLOAT16) {
      auto *fp16Data =
          reinterpret_cast<aclFloat16 *>(outputData);

      size_t elementCount =
          len / sizeof(aclFloat16);

      INFO_LOG("parse output as FP16, element count[%zu]",
               elementCount);

      for (size_t j = 0; j < elementCount; ++j) {
        float score =
            aclFloat16ToFloat(fp16Data[j]);

        resultMap.emplace(
            score,
            static_cast<unsigned int>(j));
      }
    } else if (outputType == ACL_FLOAT) {
      auto *fp32Data =
      ...
      INFO_LOG("parse output as FP32, element count[%zu]",
               elementCount);
      ...
      ERROR_LOG("unsupported output type[%d]",
                static_cast<int32_t>(outputType));
    ...
    for (auto it = resultMap.begin();
         it != resultMap.end() && count < 5;
         ++it) {
      ++count;

      INFO_LOG("top %d: index[%u] value[%f]",
               count,
               it->second,
               it->first);
    }
