...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      ret = aclrtMemcpy(outHostData, len, outputData, len, ACL_MEMCPY_DEVICE_TO_HOST);
      if (ret != ACL_SUCCESS) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outputData);
    }
  ...
  INFO_LOG("output outputData success");
  return;
}
