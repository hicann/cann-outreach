...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        auto tmp0 = tmpBuf0.Get<DTYPE_X>();
        auto tmp1 = tmpBuf1.Get<DTYPE_X>();
        auto tmp2 = tmpBuf2.Get<DTYPE_X>();

        AscendC::Exp(tmp0, xLocal, this->tileLength);              // exp(x)
        AscendC::Muls(tmp1, xLocal, (DTYPE_X)-1.0, this->tileLength); // -x
        AscendC::Exp(tmp1, tmp1, this->tileLength);                // exp(-x)
        AscendC::Add(tmp2, tmp0, tmp1, this->tileLength);          // exp(x)+exp(-x)
        AscendC::Sub(yLocal, tmp0, tmp1, this->tileLength);        // exp(x)-exp(-x)
        AscendC::Div(yLocal, yLocal, tmp2, this->tileLength);      // tanh
