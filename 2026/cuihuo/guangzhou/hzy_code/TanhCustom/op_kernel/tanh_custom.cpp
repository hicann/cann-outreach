...
public:
    ...
    {
        // 分块：totalLength均分到各核，核内再按tileNum切分，并预留double buffer
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Exp(tmp1, xLocal, this->tileLength);                   // e^x
        AscendC::Muls(tmp0, xLocal, (DTYPE_X)(-1.0), this->tileLength); // -x
        AscendC::Exp(tmp0, tmp0, this->tileLength);                     // e^-x
        AscendC::Add(tmp2, tmp1, tmp0, this->tileLength);               // e^x + e^-x
        AscendC::Sub(yLocal, tmp1, tmp0, this->tileLength);             // e^x - e^-x
        AscendC::Div(yLocal, yLocal, tmp2, this->tileLength);
