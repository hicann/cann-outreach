...
void SampleRes50ImagenetClassification::OutputModelResult()
{
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i)
  {
    ...
    // Get model output data buffer address
    void *outputBuffer = aclGetDataBufferAddr(dataBuffer);
    uint32_t len = aclGetDataBufferSizeV2(dataBuffer);

    void *outHostData = nullptr;
    aclError ret = ACL_SUCCESS;
    // Model output type is FP16, read the raw output buffer as aclFloat16
    ...
    if (!g_isDevice)
    {
      ...
      outputData = static_cast<aclFloat16 *>(outHostData);
    }
    else
    {
      outputData = static_cast<aclFloat16 *>(outputBuffer);
    }
    // Step 1: convert the FP16 output data to FP32
    float *outData = new float[len / sizeof(aclFloat16)];
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j)
    {
      outData[j] = aclFloat16ToFloat(*outputData);
      outputData++;
    }

    // Step 2: process the map result with the converted Float data
    // Step 2: process the map result with the converted Float data
