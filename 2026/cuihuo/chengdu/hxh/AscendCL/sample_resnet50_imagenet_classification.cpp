...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outData = nullptr;    // 改为 aclFloat16*
    void *outHostData = nullptr;      // 用于 host 侧临时存储
    aclError ret = ACL_SUCCESS;
    ...
    // 解析结果，FP16 转 float 并存入 map
    map<float, unsigned int, greater<float>> resultMap;
    size_t elemCount = len / sizeof(aclFloat16);  // 每个 FP16 占 2 字节
    for (unsigned int j = 0; j < elemCount; ++j) {
      float val = aclFloat16ToFloat(outData[j]);  // 关键转换
      resultMap[val] = j;
    }
