...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outHostData); // 2
    } else {
      outData = reinterpret_cast<aclFloat16 *>(data); //3
    }
    map<float, unsigned int, greater<float> > resultMap;
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) { //4
      resultMap[aclFloat16ToFloat(*outData)] = j; //5
      outData++;
    }
