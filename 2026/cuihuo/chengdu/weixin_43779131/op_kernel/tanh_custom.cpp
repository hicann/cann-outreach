...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> denom = tmpBuf2.Get<DTYPE_X>();
 
        // expPos = exp(x)
        AscendC::Exp(expPos, xLocal, this->tileLength);
        // expNeg = exp(-x)
        AscendC::Muls(expNeg, xLocal, (DTYPE_X)(-1.0), this->tileLength);
        AscendC::Exp(expNeg, expNeg, this->tileLength);
        // denom = exp(x) + exp(-x)
        AscendC::Add(denom, expPos, expNeg, this->tileLength);
        // yLocal = exp(x) - exp(-x)
        AscendC::Sub(yLocal, expPos, expNeg, this->tileLength);
        // yLocal = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        AscendC::Div(yLocal, yLocal, denom, this->tileLength);
