...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    const aclFloat16 *outData = nullptr;
    if (!g_isDevice) {
      ...
      outData = static_cast<const aclFloat16 *>(outHostData);
    } else {
      outData = static_cast<const aclFloat16 *>(data);
    }

    constexpr size_t classNum = 1000U;
    const size_t requiredBytes = classNum * sizeof(aclFloat16);
    if (len < requiredBytes) {
      ERROR_LOG("FP16 output buffer is too small, actual[%u], required[%zu]", len, requiredBytes);
      if (!g_isDevice) {
        (void)aclrtFreeHost(outHostData);
      }
      return;
    }
    INFO_LOG("parse model output as FP16, buffer size[%u]", len);

    map<float, unsigned int, greater<float> > resultMap;
    for (unsigned int j = 0; j < classNum; ++j) {
      const float outputValue = aclFloat16ToFloat(*outData);
      resultMap[outputValue] = j;
      ++outData;
    }
