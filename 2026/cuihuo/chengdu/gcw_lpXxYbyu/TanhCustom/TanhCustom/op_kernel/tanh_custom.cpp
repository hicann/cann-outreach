...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(tmp0, tmp0, 2.0f, this->tileLength);         // 2x
        AscendC::Exp(tmp1, tmp0, this->tileLength);                // e^(2x)
        AscendC::Adds(tmp2, tmp1, -1.0f, this->tileLength);        // e^(2x) - 1
        AscendC::Adds(tmp0, tmp1, 1.0f, this->tileLength);         // e^(2x) + 1
        AscendC::Div(tmp0, tmp2, tmp0, this->tileLength);          // (e^(2x) - 1) / (e^(2x) + 1)
