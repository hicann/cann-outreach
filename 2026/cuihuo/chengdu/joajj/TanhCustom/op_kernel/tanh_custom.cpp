...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        this->xGm.SetGlobalBuffer((__gm__ DTYPE_X*)x + this->blockLength * AscendC::GetBlockIdx(), this->blockLength);
        this->yGm.SetGlobalBuffer((__gm__ DTYPE_Y*)y + this->blockLength * AscendC::GetBlockIdx(), this->blockLength);
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(tmp1, xLocal, (DTYPE_X)(-1.0f), this->tileLength);
        AscendC::Exp(tmp2, tmp1, this->tileLength);
        AscendC::Sub(tmp1, tmp0, tmp2, this->tileLength);
        AscendC::Add(tmp2, tmp0, tmp2, this->tileLength);
        AscendC::Div(yLocal, tmp1, tmp2, this->tileLength);
