...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> num = tmpBuf2.Get<DTYPE_X>();
        AscendC::LocalTensor<DTYPE_X> den = tmpBuf3.Get<DTYPE_X>();
 
        // tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        AscendC::Exp(expPos, xLocal, this->tileLength);              // exp(x)
        AscendC::Muls(expNeg, xLocal, (half)-1.0, this->tileLength); // -x
        AscendC::Exp(expNeg, expNeg, this->tileLength);              // exp(-x)
        AscendC::Sub(num, expPos, expNeg, this->tileLength);         // exp(x) - exp(-x)
        AscendC::Add(den, expPos, expNeg, this->tileLength);         // exp(x) + exp(-x)
        AscendC::Div(yLocal, num, den, this->tileLength);            // (exp(x) - exp(-x)) / (exp(x) + exp(-x))
