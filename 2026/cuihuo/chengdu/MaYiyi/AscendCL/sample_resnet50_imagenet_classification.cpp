...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // 假设输出一定是 FP16（由 ATC 参数保证）
    const bool isFp16 = true;   // 强制 FP16
    size_t elemSize = sizeof(uint16_t);
    size_t numElem = len / elemSize;

    void *outHostData = nullptr;
    float *outFloatData = nullptr;
    ...
    // 强制转换 FP16 -> float
    uint16_t *fp16Data = static_cast<uint16_t*>(outHostData);
    floatBuffer.resize(numElem);
    for (size_t j = 0; j < numElem; ++j) {
      floatBuffer[j] = aclFloat16ToFloat(fp16Data[j]);
    }
    outFloatData = floatBuffer.data();
