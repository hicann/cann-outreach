...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> expPos = tmpBuf0.Get<DTYPE_X>();  // exp(x)
        AscendC::LocalTensor<DTYPE_X> expNeg = tmpBuf1.Get<DTYPE_X>();  // exp(-x)
        AscendC::LocalTensor<DTYPE_X> numerator = tmpBuf2.Get<DTYPE_X>();  // exp(x) - exp(-x)
        ...
        AscendC::Add(expPos, expPos, expNeg, this->tileLength);         // expPos = exp(x) + exp(-x) (denominator)
        AscendC::Div(yLocal, numerator, expPos, this->tileLength);      // y = numerator / denominator
