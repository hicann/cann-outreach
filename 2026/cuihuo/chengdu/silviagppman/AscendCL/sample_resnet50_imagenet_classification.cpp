...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outData = nullptr;  // float *outData = nullptr;
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outHostData);  // reinterpret_cast<float *>(outHostData)
    } else {
      outData = reinterpret_cast<aclFloat16 *>(data);  // reinterpret_cast<float *>(data)
    }
