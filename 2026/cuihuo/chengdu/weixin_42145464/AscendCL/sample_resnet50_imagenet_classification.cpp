...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outData = nullptr; //modify 1
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outHostData); //modify 2
        } else {
            // Device模式：直接使用设备上输出buffer地址，不拷贝
      outData = reinterpret_cast<aclFloat16 *>(data);        //modify 3
    }
   // map按float降序排序，key为概率值，value为类别下标
      map<float, unsigned int, greater<float> > resultMap;
       for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) { //modify 4
            resultMap[aclFloat16ToFloat(*outData)] = j;              //modify 5
            outData++;
        }
