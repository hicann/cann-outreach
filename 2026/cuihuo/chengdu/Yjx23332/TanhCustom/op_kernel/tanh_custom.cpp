...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> calc = tmpBuf2.Get<DTYPE_X>();
 	    AscendC::Muls(calc, xLocal, static_cast<DTYPE_X>(-1), this->tileLength);

 	    AscendC::Exp(expNegX, calc,this->tileLength);
 	    AscendC::Exp(expX, xLocal,this->tileLength);

 	    AscendC::Sub(yLocal,expX,expNegX,this->tileLength);
 	    AscendC::Add(calc,expX,expNegX,this->tileLength);
 	    AscendC::Div(yLocal,yLocal,calc,this->tileLength);
...
extern "C" __global__ __aicore__ void tanh_custom(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    // TODO: 考生自行补齐`
    KernelTanh kernelTanh;
    kernelTanh.Init(x, y, tilingData.totalLength, tilingData.tileNum);
    kernelTanh.Process();
}