...
#include <iostream>
#include <map>                     // 显式包含 map
#include <acl/acl.h>              // 包含 CANN API（提供 aclFloat16ToFloat 等）
...
// ========== 重写后的输出处理函数（兼容旧版 CANN） ==========
void SampleRes50ImagenetClassification::OutputModelResult() {
    size_t outputNum = aclmdlGetNumOutputs(modelDesc_->GetModelDesc());  // 获取输出数量
    for (size_t i = 0; i < outputNum; ++i) {
        ...
        // 获取该输出的维度信息以计算元素个数
        aclmdlIODims dims;
        aclError ret = aclmdlGetOutputDims(modelDesc_->GetModelDesc(), i, &dims);
        if (ret != ACL_SUCCESS) {
            ERROR_LOG("aclmdlGetOutputDims failed for index %zu", i);
            return;
        }
        size_t elemCount = 1;
        for (size_t d = 0; d < dims.dimCount; ++d) {
            elemCount *= dims.dims[d];
        }

        void *outHostData = nullptr;
        // 若运行在 Host 侧，需将数据从 Device 拷贝到 Host
        if (!g_isDevice) {
            ret = aclrtMallocHost(&outHostData, len);
            if (ret != ACL_SUCCESS) {
                ERROR_LOG("aclrtMallocHost failed, len[%u], errorCode[%d]", len, static_cast<int32_t>(ret));
                return;
            }
            ...
            outHostData = data;   // Device 侧运行，直接使用设备内存指针
        }

        // 因为 ATC 转换时指定了 --output_type=FP16，所以输出数据是 FP16（半精度）
        // 将 uint16_t 表示的 half 转换为 float 并存入 map 排序
        uint16_t *halfData = reinterpret_cast<uint16_t *>(outHostData);
        map<float, unsigned int, greater<float>> resultMap;
        for (unsigned int j = 0; j < elemCount; ++j) {
            float val = aclFloat16ToFloat(halfData[j]);   // CANN 提供的转换函数
            resultMap[val] = j;
        }
