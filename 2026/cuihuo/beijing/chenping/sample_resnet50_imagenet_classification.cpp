...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // 转换为aclFloat16指针，适配FP16模型输出
    aclFloat16* outputData = static_cast<aclFloat16*>(outputBuffer);
    uint32_t len = aclGetDataBufferSizeV2(dataBuffer);

    void *outHostData = nullptr;
    aclError ret = ACL_SUCCESS;
    aclFloat16 *hostFp16Data = nullptr;
    ...
    if (!g_isDevice) {
      ...
      hostFp16Data = reinterpret_cast<aclFloat16*>(outHostData);
    } else {
      hostFp16Data = outputData;
    }
    ...
    for (unsigned int j = 0; j < elemCount; ++j) {
      // FP16 转为 FP32浮点参与排序
      float score = aclFloat16ToFloat(hostFp16Data[j]);
      resultMap[score] = j;
    }
