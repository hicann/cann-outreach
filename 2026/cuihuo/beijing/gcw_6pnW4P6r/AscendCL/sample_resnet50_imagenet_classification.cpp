...
void SampleRes50ImagenetClassification::OutputModelResult() {
    for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
        // Get model output data. The OM output is FP16, so interpret each
        // element as aclFloat16 and convert it to FP32 before ranking.
        ...
        const aclFloat16 *outputData = nullptr;
        if (!g_isDevice) {
            ret = aclrtMallocHost(&outHostData, len);
            if (ret != ACL_SUCCESS) {
                ERROR_LOG("aclrtMallocHost failed, malloc len[%u], errorCode[%d]", len,
            ...
            outputData = static_cast<const aclFloat16 *>(outHostData);
        } else {
            outputData = static_cast<const aclFloat16 *>(outputBuffer);
        }

        multimap<float, unsigned int, greater<float>> resultMap;
        const size_t outputCount = len / sizeof(aclFloat16);
        for (size_t j = 0; j < outputCount; ++j) {
            const float value = aclFloat16ToFloat(outputData[j]);
            resultMap.emplace(value, static_cast<unsigned int>(j));
        }
...
Result SampleRes50ImagenetClassification::Process() {
  ...
  for (size_t index = 0; index < sizeof(testFile) / sizeof(testFile[0]); ++index) {
    ...
    if (aclRet != ACL_SUCCESS) {
      ERROR_LOG("aclmdlExecute failed, errorCode[%d]", static_cast<int32_t>(aclRet));
      aclrtFree(picDevBuffer);
      return FAILED;
    }
