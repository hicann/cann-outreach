...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outFp16Data = reinterpret_cast<uint16_t *>(outHostData);
    } else {
      outFp16Data = reinterpret_cast<uint16_t *>(data);
    }

    size_t elemNum = len / sizeof(uint16_t);
    map<float, unsigned int, greater<float> > resultMap;

    for (unsigned int j = 0; j < elemNum; ++j) {
        // 适配 CANN9.0.0 接口
        aclFloat16 fp16Val = static_cast<aclFloat16>(outFp16Data[j]);
        float val = aclFloat16ToFloat(fp16Val);
        resultMap[val] = j;
    }
