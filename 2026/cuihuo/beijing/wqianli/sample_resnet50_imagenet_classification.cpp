...
class SampleRes50ImagenetClassification {
 public:
  SampleRes50ImagenetClassification() : modelId_(0) {}
...
Result SampleRes50ImagenetClassification::PrepareModel(const char *modelPath) {
  ...
  if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclmdlQuerySize failed, errorCode[%d]", static_cast<int32_t>(ret));
    return FAILED;
  }
  ...
  if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclmdlLoadFromFileWithMem failed, errorCode[%d]", static_cast<int32_t>(ret));
    return FAILED;
  }
...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outData = static_cast<aclFloat16 *>(outHostData);
    } else {
      outData = static_cast<aclFloat16 *>(data);
    }
    map<float, unsigned int, greater<float> > resultMap;
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      resultMap[aclFloat16ToFloat(*outData)] = j;
      outData++;
    }
...
Result SampleRes50ImagenetClassification::Process() {
  ...
  for (size_t index = 0; index < sizeof(testFile) / sizeof(testFile[0]); ++index) {
    ...
    if (aclRet != ACL_SUCCESS) {
      aclrtFree(picDevBuffer);
      ERROR_LOG("aclmdlExecute failed, errorCode[%d], index is %zu", static_cast<int32_t>(aclRet), index);
      return FAILED;
    }
