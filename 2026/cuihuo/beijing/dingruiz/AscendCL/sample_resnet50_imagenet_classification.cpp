...
// ------------------------------------------------------------
// 严格按照指导修改：输出数据按 aclFloat16 解释，并转为 float
// ------------------------------------------------------------
void SampleRes50ImagenetClassification::OutputModelResult() {
    aclmdlDataset *dataset = modelOutput_->GetDataSet();
    size_t numOutputs = aclmdlGetDatasetNumBuffers(dataset);
    ...
    for (size_t i = 0; i < numOutputs; ++i) {
        ...
        if (!g_isDevice) {
            ...
            outHostData = data;   // 设备侧直接使用
        }

        // 指导步骤1：将输出缓冲区转换为 aclFloat16* 类型
        aclFloat16* outputData = static_cast<aclFloat16*>(outHostData);
        uint32_t elemCount = len / sizeof(aclFloat16);

        // 使用 map 排序，key 为 float 值，value 为索引
        std::map<float, unsigned int, std::greater<float>> resultMap;
        for (unsigned int j = 0; j < elemCount; ++j) {
            // 指导步骤2：将 aclFloat16 转为 float
            float val = aclFloat16ToFloat(outputData[j]);
            resultMap[val] = j;
        }
        ...
        if (!g_isDevice && outHostData != nullptr) {
            aclrtFreeHost(outHostData);
        }
    }
