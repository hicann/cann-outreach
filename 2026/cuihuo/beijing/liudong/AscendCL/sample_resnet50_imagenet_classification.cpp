...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outDataFp16 = nullptr; //【修改】FP16指针
    ...
    if (!g_isDevice) {
      ...
      outDataFp16 = reinterpret_cast<aclFloat16 *>(outHostData); //【修改】转为FP16指针
    } else {
      outDataFp16 = reinterpret_cast<aclFloat16 *>(data); //【修改】device侧直接绑定FP16
    }

    map<float, unsigned int, greater<float> > resultMap;
    //【修改】元素个数使用aclFloat16宽度计算
    unsigned int elemCount = len / sizeof(aclFloat16);
    for (unsigned int j = 0; j < elemCount; ++j) {
      //【核心】FP16转FP32
      float score = aclFloat16ToFloat(outDataFp16[j]);
      resultMap[score] = j;
    }
...
int main() {
  ...
  // are released before the process exits.
  {
    ...
    ret = sampleRes50.PrepareModel("../model/resnet50_32.om");
    if (ret != SUCCESS) {
