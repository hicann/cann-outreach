...
{
    for (size_t i = 0;
         i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet());
         ++i) {
        ...
        if (!g_isDevice) {
            ...
            if (ret != ACL_SUCCESS) {
                ERROR_LOG(
                    "aclrtMemcpy failed, errorCode[%d]",
                    static_cast<int32_t>(ret));
        ...
        // 注意这里 sizeof(aclFloat16)，不再是 sizeof(float)
        ...
        for (unsigned int j = 0; j < elementNum; ++j) {
            ...
            // FP16 -> FP32
            float score = aclFloat16ToFloat(outData[j]);

            resultMap.emplace(score, j);
        }
