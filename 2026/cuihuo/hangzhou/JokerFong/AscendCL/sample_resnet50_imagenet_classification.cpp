...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outHostData);  // 直接赋值，不要重新定义
    } else {
      outData = reinterpret_cast<aclFloat16 *>(data);  // 直接赋值，不要重新定义
    }
    
    map<float, unsigned int, greater<float> > resultMap;
    // 使用 sizeof(aclFloat16) 而不是 sizeof(float)
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      // 将 aclFloat16 转换为 float
