...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    aclFloat16 *outData = nullptr;  // 修改1：float* 改成 aclFloat16*
    if (!g_isDevice) {
      ...
      outData = reinterpret_cast<aclFloat16 *>(outHostData);  // 修改2：强制转换类型改 aclFloat16*
    } else {
      outData = reinterpret_cast<aclFloat16 *>(data);         // 修改2：同上
    }
    map<float, unsigned int, greater<float> > resultMap;
    // 修改3：每个元素大小改为 sizeof(aclFloat16)，取值时用 aclFloat16ToFloat 转回 float
