...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // 【任务2 修改】模型输出类型已由 atc 命令 --output_type=FP16 改为 FP16，
    // 因此读取输出时缓冲区指针需按 aclFloat16 类型解析。
    ...
    if (!g_isDevice) {
      ...
      // 关键修改：输出为 aclFloat16，按 aclFloat16* 解析缓冲区地址
      outData = reinterpret_cast<aclFloat16 *>(outHostData);
    } else {
      // 关键修改：输出为 aclFloat16，按 aclFloat16* 解析缓冲区地址
      outData = reinterpret_cast<aclFloat16 *>(data);
    }
    // 【任务2 修改】结果 map 仍以 float 为键排序：逐个把 aclFloat16 转成 float 后再入 map。
    map<float, unsigned int, greater<float> > resultMap;
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      // outData 是 aclFloat16 的数据，将其转成 float
      float value = aclFloat16ToFloat(outData[j]);
      resultMap[value] = j;
    }
