...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
    ...
    // =====================任务2修改1：输出数据类型改为aclFloat16适配FP16模型输出=====================
    // 原代码：float *outData = nullptr;
    ...
    if (!g_isDevice) {
      ...
      // =====================任务2修改2：强转类型从float*改为aclFloat16*=====================
      // 原代码：outData = reinterpret_cast<float *>(outHostData);
      outData = reinterpret_cast<aclFloat16 *>(outHostData);
      // =================================================================================
    } else {
      // =====================任务2修改3：设备侧指针同步改为aclFloat16*=====================
      // 原代码：outData = reinterpret_cast<float *>(data);
      outData = reinterpret_cast<aclFloat16 *>(data);
      // ================================================================================
    }
    map<float, unsigned int, greater<float> > resultMap;
    // =====================任务2修改4：元素个数按aclFloat16(2字节)计算，增加FP16转float逻辑=====================
    // 原代码：for (unsigned int j = 0; j < len / sizeof(float); ++j) { resultMap[*outData] = j; outData++; }
    for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {
      // 将FP16半精度数值转换为float单精度，用于排序与打印
      float score = aclFloat16ToFloat(*outData);
      resultMap[score] = j;
      outData++;
    }
    // =====================================================================================================
