...
void SampleRes50ImagenetClassification::OutputModelResult() {
  for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(modelOutput_->GetDataSet()); ++i) {
        ...
        aclFloat16 *outData = nullptr;  // 改为 aclFloat16 类型
        
        if (!g_isDevice) {
            // 如果在Host端运行，需要从Device拷贝数据到Host
            ...
            outData = static_cast<aclFloat16 *>(outHostData);  // 转为 aclFloat16*
        } else {
            outData = static_cast<aclFloat16 *>(data);  // 直接在Device上读取，转为 aclFloat16*
        }

        // 构建排序结果映射表
        map<float, unsigned int, greater<float>> resultMap;
        for (unsigned int j = 0; j < len / sizeof(aclFloat16); ++j) {  // 注意：sizeof(aclFloat16)
            // 关键修改：将 FP16 转为 float 后再插入 map
            float floatValue = aclFloat16ToFloat(outData[j]);
            resultMap[floatValue] = j;
        }
