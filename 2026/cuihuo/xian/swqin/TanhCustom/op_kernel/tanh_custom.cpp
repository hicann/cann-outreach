...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_Y> expX = tmpBuf0.Get<DTYPE_Y>();
        AscendC::LocalTensor<DTYPE_Y> expNegX = tmpBuf1.Get<DTYPE_Y>();
        AscendC::LocalTensor<DTYPE_Y> denominator = tmpBuf2.Get<DTYPE_Y>();

        // tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        AscendC::Exp(expX, xLocal, this->tileLength);
        AscendC::Muls(yLocal, xLocal, static_cast<DTYPE_Y>(-1.0), this->tileLength);
        AscendC::Exp(expNegX, yLocal, this->tileLength);
        AscendC::Sub(yLocal, expX, expNegX, this->tileLength);
        AscendC::Add(denominator, expX, expNegX, this->tileLength);
        AscendC::Div(yLocal, yLocal, denominator, this->tileLength);
