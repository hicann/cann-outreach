...
private:
    ...
    {
        // 3. 补齐 Compute：计算 tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        ...
        AscendC::LocalTensor<DTYPE_X> num = tmpBuf2.Get<DTYPE_X>(); // 用于存分子
        ...
        // (3) 分子 = exp(x) - exp(-x)
        AscendC::Sub(num, expX, expNegX, this->tileLength);
        
        // (4) 分母 = exp(x) + exp(-x) -> 重复利用 expX 来存分母，节约内存
        AscendC::Add(expX, expX, expNegX, this->tileLength);
        
        // (5) 最终结果 = 分子 / 分母
        AscendC::Div(yLocal, num, expX, this->tileLength);
