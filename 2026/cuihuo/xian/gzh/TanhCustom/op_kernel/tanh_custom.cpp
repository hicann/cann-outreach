...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> t0 = tmpBuf0.Get<DTYPE_X>();  // e^x
        AscendC::LocalTensor<DTYPE_X> t1 = tmpBuf1.Get<DTYPE_X>();  // e^-x
        AscendC::LocalTensor<DTYPE_X> t2 = tmpBuf2.Get<DTYPE_X>();  // numerator

        AscendC::Exp(t0, xLocal, this->tileLength);
        AscendC::Muls(t1, xLocal, (DTYPE_X)-1, this->tileLength);
        AscendC::Exp(t1, t1, this->tileLength);
        AscendC::Sub(t2, t0, t1, this->tileLength);   // e^x - e^-x
        AscendC::Add(t1, t0, t1, this->tileLength);   // e^x + e^-x  (复用 t1)
