...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Exp(tmp1, tmp0, this->tileLength);          // exp(x)
        AscendC::Muls(tmp2, tmp0, -1.0f, this->tileLength);  // -x
        AscendC::Exp(tmp2, tmp2, this->tileLength);          // exp(-x)

        AscendC::Sub(tmp0, tmp1, tmp2, this->tileLength);    // exp(x) - exp(-x)
        AscendC::Add(tmp1, tmp1, tmp2, this->tileLength);    // exp(x) + exp(-x)
        AscendC::Div(tmp0, tmp0, tmp1, this->tileLength);    // tanh(x)

        AscendC::Cast(yLocal, tmp0, AscendC::RoundMode::CAST_NONE, this->tileLength);
