...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(tmp1, xLocal, static_cast<DTYPE_X>(-1.0f), this->tileLength);

        AscendC::Exp(tmp1, tmp1, this->tileLength);

        AscendC::Sub(yLocal, tmp0, tmp1, this->tileLength);

        AscendC::Add(tmp2, tmp0, tmp1, this->tileLength);
