...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        half negOne = (half)-1.0;

        // tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        AscendC::Exp(expX, xLocal, this->tileLength);
        AscendC::Muls(expNX, xLocal, negOne, this->tileLength);
        AscendC::Exp(expNX, expNX, this->tileLength);
        AscendC::Sub(numerator, expX, expNX, this->tileLength);
