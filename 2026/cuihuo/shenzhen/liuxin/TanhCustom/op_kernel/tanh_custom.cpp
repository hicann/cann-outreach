...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Neg(negX, xLocal, this->tileLength);
        AscendC::Exp(expNegX, negX, this->tileLength);

        AscendC::LocalTensor<DTYPE_X> numerator = tmpBuf1.Get<DTYPE_X>();
        AscendC::Sub(numerator, expX, expNegX, this->tileLength);

        AscendC::LocalTensor<DTYPE_X> denominator = tmpBuf2.Get<DTYPE_X>();
        AscendC::Add(denominator, expX, expNegX, this->tileLength);

        AscendC::Div(yLocal, numerator, denominator, this->tileLength);
