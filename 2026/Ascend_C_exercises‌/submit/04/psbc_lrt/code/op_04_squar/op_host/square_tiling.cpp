...
static ge::graphStatus SquareTilingFunc(gert::TilingContext* context)
{
    ...
    // 注意：TilingContext 的 GetInputShape 返回的是 StorageShape*，
    // 需通过 GetOriginShape() 才能得到 gert::Shape。
    const gert::StorageShape* inputStorageShapePtr = context->GetInputShape(0);
    OP_CHECK_NULL_WITH_CONTEXT(context, inputStorageShapePtr);
    gert::Shape inputShape = EnsureNotScalar(inputStorageShapePtr->GetOriginShape());
    int64_t totalNum = static_cast<int64_t>(inputShape.GetShapeSize());
    if (totalNum < 1) {
        totalNum = 1;
    }
    ...
    int64_t dtypeSize = (dtype == ge::DT_FLOAT16 || dtype == ge::DT_BF16) ? 2 : 4;

    // 32 字节对齐所需的元素数
    int64_t ubBlockSize = static_cast<int64_t>(GetUbBlockSize(context));
    int64_t alignNum = ubBlockSize / dtypeSize;
    if (alignNum < 1) {
        alignNum = 1;
    }

    // 每个核处理元素数：向上对齐到 alignNum 的倍数
    ...
    // 每次 UB 循环处理元素数（向下对齐到 alignNum 的倍数）
    int64_t ubAvailable = static_cast<int64_t>(ubSize) / (2 * BUFFER_NUM * dtypeSize);
    int64_t ubFactor = FloorAlign(ubAvailable, alignNum);
    if (ubFactor <= 0) {
        ubFactor = alignNum;
    }
