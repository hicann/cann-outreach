...
TEST_F(SquareKernelTest, test_kernel_run)
{
    ...
    ICPU_RUN_KF((square<1>), numBlocks, x, y, workspace, tiling);
    ...
    { std::ofstream _ofs("float32_output_square_0.bin", std::ios::binary); _ofs.write(reinterpret_cast<const char*>(yHost.data()), yByteSize); }
