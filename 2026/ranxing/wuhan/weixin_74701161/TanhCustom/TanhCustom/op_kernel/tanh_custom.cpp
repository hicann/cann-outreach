...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Exp(tmpTensor0, xLocal, this->tileLength);                // tmpTensor0 = e^x
        AscendC::Muls(tmpTensor1, xLocal, (half)-1.0, this->tileLength);   // tmpTensor1 = -x
        AscendC::Exp(tmpTensor1, tmpTensor1, this->tileLength);            // tmpTensor1 = e^{-x}
        AscendC::Sub(tmpTensor2, tmpTensor0, tmpTensor1, this->tileLength);// tmpTensor2 = e^x - e^{-x}
        AscendC::Add(tmpTensor1, tmpTensor0, tmpTensor1, this->tileLength);// tmpTensor1 = e^x + e^{-x}
        AscendC::Div(yLocal, tmpTensor2, tmpTensor1, this->tileLength);    // y = (e^x - e^{-x}) / (e^x + e^{-x})
