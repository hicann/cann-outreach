...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    // uint32_t totalLength = 1024;
    // const uint32_t TILE_NUM = 2;

    // tiling.totalLength = totalLength;
    // tiling.tileNum = TILE_NUM;

    // // ✅ 显式强制转换 void* → uint8_t*
    // uint8_t* buf = reinterpret_cast<uint8_t*>(context->GetRawTilingData()->GetData());
    // size_t bufCap = context->GetRawTilingData()->GetCapacity();
    // size_t tilingSize = sizeof(TanhCustomTilingData);

    // if (tilingSize > bufCap) {
    //     return ge::GRAPH_FAILED;
    // }

    // memcpy(buf, &tiling, tilingSize);
