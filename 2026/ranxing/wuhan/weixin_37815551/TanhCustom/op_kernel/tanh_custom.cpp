...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        // tmpTensor0 = e^{-x}
        Muls(tmpTensor0, xLocal, (DTYPE_X)-1.0f, this->tileLength);
        Exp(tmpTensor0, tmpTensor0, this->tileLength);
        // tmpTensor1 = e^{x}
        Exp(tmpTensor1, xLocal, this->tileLength);
        // y = e^{x} - e^{-x}
        Sub(yLocal, tmpTensor1, tmpTensor0, this->tileLength);
        // tmpTensor0 = e^{x} + e^{-x}
        Add(tmpTensor0, tmpTensor1, tmpTensor0, this->tileLength);
        // y = (e^{x} - e^{-x}) / (e^{x} + e^{-x}) = tanh(x)
        Div(yLocal, yLocal, tmpTensor0, this->tileLength);
