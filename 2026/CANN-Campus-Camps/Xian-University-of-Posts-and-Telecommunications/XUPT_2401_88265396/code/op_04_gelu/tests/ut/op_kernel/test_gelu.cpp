...
class GeluKernelTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        cout << "GeluKernelTest SetUp" << endl;
    }
    static void TearDownTestCase()
    {
        cout << "GeluKernelTest TearDown" << endl;
    }
};

TEST_F(GeluKernelTest, test_kernel_run)
{
    constexpr size_t size = 32;
    constexpr size_t tilingDataSize = sizeof(GeluTilingData);
    ...
    //         参考 op_kernel/gelu_tiling_data.h 中的字段定义
    //         参考 op_host/gelu_tiling.cpp 中的 tiling 计算逻辑
    GeluTilingData* tilingData = reinterpret_cast<GeluTilingData*>(tiling);
    tilingData->totalNum = size;
    tilingData->blockFactor = size;
    tilingData->ubFactor = size;
    
    // TODO: tilingKey 应与 op_host/gelu_tiling.cpp 中 SetTilingKey 设置的值一致
    ICPU_SET_TILING_KEY(0);
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    
    ICPU_RUN_KF((gelu<0>), numBlocks, input_x, output, workspace, tiling);
    ...
    { std::ofstream _ofs("float16_output_gelu_0.bin", std::ios::binary); _ofs.write(reinterpret_cast<const char*>(outputHost.data()), outputByteSize); }
