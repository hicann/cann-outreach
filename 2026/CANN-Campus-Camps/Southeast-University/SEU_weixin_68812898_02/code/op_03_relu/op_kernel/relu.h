...
constexpr int64_t BLOCK_ELEMS = 32; // 按 32 元素对齐，保证 32B 对齐（half 下也满足）
...
__aicore__ inline void Relu<T>::Init(GM_ADDR x, GM_ADDR y, const ReluTilingData* tilingData)
{
    ...
    int64_t offset = static_cast<int64_t>(GetBlockIdx()) * blockFactor;
    int64_t remain = totalNum - offset;
