...
{
    int64_t blockOffset = tilingData->blockFactor * static_cast<int64_t>(AscendC::GetBlockIdx());
    int64_t remainingLength = tilingData->totalNum - blockOffset;
    blockLength_ = (remainingLength <= 0)
        ? 0
        : ((remainingLength < tilingData->blockFactor) ? remainingLength : tilingData->blockFactor);
...
__aicore__ inline void Relu<T>::Process()
{
    ...
    for (int64_t i = 0; i < loopCount; ++i) {
        int64_t remainNum = blockLength_ - i * ubLength_;
        int64_t currentNum = (remainNum < ubLength_) ? remainNum : ubLength_;
        CopyIn(i, currentNum);
        Compute(currentNum);
