...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength) {
        ...
        uint32_t baseLen = totalLength / BLOCK_NUM;
        uint32_t remainLen = totalLength % BLOCK_NUM;

        // 余数交给最后一个block处理
        if(blockIdx == BLOCK_NUM - 1) {
            this->blockLength = baseLen + remainLen;
        } else {
            this->blockLength = baseLen;
        }

        // 只按双缓冲切分，不再额外8次细分，放大单次计算长度
        this->tileLength = this->blockLength / BUFFER_NUM;

        uint32_t gmOffset = baseLen * blockIdx;
        xGm.SetGlobalBuffer((__gm__ DT_X*)x + gmOffset, this->blockLength);
        yGm.SetGlobalBuffer((__gm__ DT_X*)y + gmOffset, this->blockLength);
        zGm.SetGlobalBuffer((__gm__ DT_X*)z + gmOffset, this->blockLength);
    ...
    __aicore__ inline void Process() {
        // 循环次数 = BUFFER_NUM，只有2轮，减少迭代开销，解决超时
