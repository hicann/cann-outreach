...
__aicore__ inline void Relu<T>::Init(GM_ADDR x, GM_ADDR y, const ReluTilingData* tilingData)
{
    ...
    inputGMX.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(x) + blockIdx * blockLength_, blockLength_);
    outputGMY.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(y) + blockIdx * blockLength_, blockLength_);
...
__aicore__ inline void Relu<T>::Compute(int64_t currentNum)
{
    ...
    AscendC::Relu(yLocal, xLocal, currentNum);   // y = max(0, x)，全限定避开类名遮蔽
...
__aicore__ inline void Relu<T>::Process()
{
    ...
    int64_t rem = blockLength_ % ubLength_;
    ...
    if (rem > 0) {
        CopyIn(loopCount, rem);
        Compute(rem);
        CopyOut(loopCount, rem);
    }
}
