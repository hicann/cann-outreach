...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    uint64_t totalNum = inputTensor->GetShapeSize();
    ...
    if (totalNum >= MIN_SPLIT_THRESHOLD) {
        blockDim = CeilDiv(
            ...
            MIN_SPLIT_THRESHOLD);
        blockDim = blockDim > coreNum ? coreNum : blockDim;
    }

    int64_t blockFactor = CeilDiv(
        static_cast<int64_t>(totalNum),
        blockDim);
    blockFactor = CeilAlign(blockFactor, static_cast<int64_t>(8));

    int64_t maxUbFactor =
        static_cast<int64_t>(ubSize) / TYPE_SIZE / 4;
    maxUbFactor =
        FloorAlign(maxUbFactor, static_cast<int64_t>(8));

    int64_t ubFactor =
        blockFactor < maxUbFactor ? blockFactor : maxUbFactor;
