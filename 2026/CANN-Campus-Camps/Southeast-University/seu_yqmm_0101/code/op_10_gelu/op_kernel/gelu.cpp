...
class KernelGelu {
public:
    // One vector-friendly 1K tile per core for the usual N <= 10240 case.
    // Two queues permit DMA for the next tile to overlap vector calculation
    // when a large batched tensor gives a core multiple tiles.
...
private:
    __aicore__ inline void CopyIn(uint32_t pos, uint32_t count) {
        ...
        // DataCopyPad transfers exactly count elements.  Padding makes the UB
        // vector length 32-byte aligned without reading past the tensor tail.
