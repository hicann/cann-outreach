...
constexpr int32_t BUFFER_NUM = 2;  // 每个队列的缓冲区个数（double buffer）
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum) {
        // 每核处理的数据长度：totalLength / 核数
        this->blockLength = totalLength / AscendC::GetBlockNum();
        this->tileNum = tileNum;
        // 每块长度：blockLength / tileNum / BUFFER_NUM
...
private:
    ...
    uint32_t blockLength;  // 每核处理的总长度
    uint32_t tileNum;      // 每核内分块数
