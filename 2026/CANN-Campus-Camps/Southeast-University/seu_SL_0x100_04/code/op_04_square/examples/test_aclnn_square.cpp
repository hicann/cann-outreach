...
int main()
{
    ...
    std::vector<int64_t> input_xShape = {32};
    std::vector<float> input_xHostData(32, 1);
    ...
    std::vector<float> outputHostData(32, 0);
    ret = CreateAclTensor(outputHostData, outputShape, &outputDeviceAddr, aclDataType::ACL_FLOAT16, &output);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    

    // 调用 aclnnSquareGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnSquareGetWorkspaceSize(input_x, output, &workspaceSize, &executor);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnSquareGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnSquare 第二段接口
    ret = aclnnSquare(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnSquare failed. ERROR: %d\n", ret); return ret);
