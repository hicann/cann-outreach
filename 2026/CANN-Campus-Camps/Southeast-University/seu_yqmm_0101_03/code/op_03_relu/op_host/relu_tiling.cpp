...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    ge::DataType dtype = inputDescPtr->GetDataType();
    const bool isHalf = (dtype == ge::DT_FLOAT16 || dtype == ge::DT_BF16);
    const int64_t typeSize = isHalf ? 2 : TYPE_SIZE;

    const int64_t alignNum = GetUbBlockSize(context) / typeSize;

    if (CeilDiv(totalNum, coreNum) < MIN_SPLIT_THRESHOLD) {
        coreNum = CeilDiv(totalNum, MIN_SPLIT_THRESHOLD);
    }

    const int64_t blockFactor = CeilAlign(CeilDiv(totalNum, coreNum), alignNum);

    const uint64_t ubDataSize = (ubSize / 4) * 3;
    int64_t ubFactor = FloorAlign((int64_t)(ubDataSize / (4 * typeSize)), alignNum);
