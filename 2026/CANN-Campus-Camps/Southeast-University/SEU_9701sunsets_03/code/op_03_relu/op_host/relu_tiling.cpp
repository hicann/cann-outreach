...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    const gert::Shape xShape = EnsureNotScalar(inputShape->GetStorageShape());
    ...
    int64_t typeSize = (inputDtype == ge::DT_FLOAT16) ? 2 : TYPE_SIZE;
    int64_t blockAlign = 32 / typeSize;
    ...
    int64_t ubFactor = static_cast<int64_t>(ubSize) / (DOUBLE_BUFFER_NUM * 2 * typeSize);
    ubFactor = FloorAlign(ubFactor, blockAlign);
    ubFactor = ubFactor > blockFactor ? blockFactor : ubFactor;
    ubFactor = ubFactor <= 0 ? blockAlign : ubFactor;
