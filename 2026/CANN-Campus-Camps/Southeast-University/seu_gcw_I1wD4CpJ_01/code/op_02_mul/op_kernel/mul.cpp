...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t length, uint32_t tileNum) {
        ...
        // 32B 对齐：fp32=8元素, fp16=16元素
        uint32_t elemAlign = 32 / static_cast<uint32_t>(sizeof(DT_X));
        uint32_t t = this->coreLength / tileNum / BUFFER_NUM;
        this->tileLength = (t / elemAlign) * elemAlign;
        if (this->tileLength < elemAlign) this->tileLength = elemAlign;
