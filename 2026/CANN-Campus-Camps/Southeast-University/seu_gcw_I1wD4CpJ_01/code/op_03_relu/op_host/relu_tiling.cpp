...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    // ① 总元素数
    int64_t totalNum = context->GetInputShape(0)->GetOriginShape().GetShapeSize();
    ...
    int64_t blockElem = 32 / typeSize;                    // fp32=8, fp16=16
    int64_t blockFactor = CeilDiv(totalNum, coreNum);
    blockFactor = CeilAlign(blockFactor, blockElem);      // ★ 对齐到 32B
    tiling->blockFactor = blockFactor;

    // ③ ubFactor = blockFactor：每核一块、无尾块（先求正确）
