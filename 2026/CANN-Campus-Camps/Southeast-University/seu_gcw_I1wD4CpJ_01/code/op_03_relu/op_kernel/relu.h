/*! \file relu.h \brief Relu 算子 kernel 类定义 */
#ifndef RELU_H
#define RELU_H
...
__aicore__ inline void Relu<T>::Init(GM_ADDR x, GM_ADDR y, const ReluTilingData* tilingData)
{
    ...
    inputGMX.SetGlobalBuffer((__gm__ T*)x + this->blockLength_ * blockIdx, this->blockLength_);
    outputGMY.SetGlobalBuffer((__gm__ T*)y + this->blockLength_ * blockIdx, this->blockLength_);
...
__aicore__ inline void Relu<T>::Process()
{
    ...
    for (int64_t i = 0; i < loopCount; i++) {
        int64_t remaining = this->blockLength_ - i * this->ubLength_;
        int64_t currentNum = remaining < this->ubLength_ ? remaining : this->ubLength_;
        CopyIn(i, currentNum);
        Compute(currentNum);
