 ...
 * \brief Relu: aligned multicore tiling and double-buffer UB allocation.
 */
#include <algorithm>
#include <cstdint>
#include <limits>
...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(dtype != ge::DT_FLOAT && dtype != ge::DT_FLOAT16,
                OP_LOGE(context, "Relu only supports FP16 and FP32"),
                return ge::GRAPH_FAILED);
    ...
    const int64_t totalNum = shape.GetDimNum() == 0 ? 1 : shape.GetShapeSize();
    ...
    rawTiling->SetDataSize(sizeof(ReluTilingData));
...
IMPL_OP_OPTILING(Relu).Tiling(ReluTilingFunc)
