...
constexpr int64_t MAX_BLOCK_NUM = 8;
...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    if (blockDim > MAX_BLOCK_NUM) {
        blockDim = MAX_BLOCK_NUM;
    }
    if (blockDim > static_cast<uint32_t>(totalNum)) {
        blockDim = static_cast<uint32_t>(totalNum);
    }
    while (blockDim > 1U && (totalNum % blockDim) != 0) {
        --blockDim;
    }

    const int64_t blockFactor = totalNum / static_cast<int64_t>(blockDim);
    const int64_t typeSize = (inputDesc->GetDataType() == ge::DT_FLOAT16) ? 2 : 4;
    const int64_t alignElements = 32 / typeSize;

    int64_t maxUbFactor = static_cast<int64_t>(ubSize / 4U) / typeSize;
    maxUbFactor = (maxUbFactor / alignElements) * alignElements;
    if (maxUbFactor <= 0) {
        maxUbFactor = alignElements;
    }
