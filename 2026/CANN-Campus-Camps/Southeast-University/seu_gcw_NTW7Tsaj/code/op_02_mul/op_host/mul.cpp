...
namespace optiling {
    const uint32_t BUFFER_NUM = 2;          // 与Kernel侧保持一致
    const uint32_t UB_RESERVE = 4 * 1024;   // 预留UB空间
    ...
    static ge::graphStatus TilingFunc(gert::TilingContext *context) {
        ...
        // 计算启动核数（不超过总元素数，且上限8核）
        uint32_t block_dim = (num_cores_aiv == 0) ? 1 : num_cores_aiv;
        if (block_dim > total_length && total_length > 0) block_dim = total_length;
        const uint32_t BLOCK_DIM_CAP = 8;
        if (block_dim > BLOCK_DIM_CAP) block_dim = BLOCK_DIM_CAP;

        // 每个核心处理的数据长度（对齐到32B）
        uint32_t elem_per_block = 32 / dtype_size;  // 32B对齐
        ...
        if (block_length == 0) block_length = elem_per_block;

        // 根据UB大小计算每个tile的元素数（考虑双缓冲x,y,z三份）
        ...
        if (tile_length > ub_max_elem && ub_max_elem >= elem_per_block) tile_length = ub_max_elem;
        if (tile_length < elem_per_block) tile_length = elem_per_block;
