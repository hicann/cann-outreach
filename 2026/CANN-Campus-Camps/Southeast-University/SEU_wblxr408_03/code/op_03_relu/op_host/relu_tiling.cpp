...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    const int64_t totalNum = EnsureNotScalar(inputShape->GetOriginShape()).GetShapeSize();
    ...
    const int64_t elementSize =
        (inputDesc != nullptr && inputDesc->GetDataType() == ge::DT_FLOAT16) ? 2 : TYPE_SIZE;

    const int64_t requiredCoreNum = CeilDiv(totalNum, MIN_SPLIT_THRESHOLD);
    const int64_t blockDim = (requiredCoreNum < coreNum) ? requiredCoreNum : coreNum;
    const int64_t vectorAlign = 32 / elementSize;
    const int64_t blockFactor = CeilAlign(CeilDiv(totalNum, blockDim), vectorAlign);

    constexpr int64_t TOTAL_QUEUES = 2;
    constexpr int64_t BUFFER_NUM = 2;
    const int64_t ubFactor = FloorAlign(
        static_cast<int64_t>(ubSize) / (elementSize * TOTAL_QUEUES * BUFFER_NUM), vectorAlign);
    OP_CHECK_IF(ubFactor <= 0, OP_LOGE(context, "UB is too small for double buffering"), return ge::GRAPH_FAILED);
