...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    // 设置 Tiling 数据，适配 Double Buffer
    tiling->totalNum = totalLength;
    tiling->blockFactor = totalLength / blockDim;       // 每个 Core 处理的数据量
    tiling->ubFactor = tiling->blockFactor / 2;         // 每个 Buffer 块的数据量（BUFFER_NUM=2）
