...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    // total element count from input shape
    ...
    // prefer up to 8 cores, each core gets an equal part
    int64_t blockDim = (coreNum < 8) ? coreNum : 8;
    while (blockDim > 0 && totalNum % blockDim != 0) {
        blockDim--;
    }
    if (blockDim <= 0) {
        blockDim = 1;
    }
    int64_t blockFactor = (blockDim == 0) ? totalNum : totalNum / blockDim;
    if (blockFactor == 0) {
        blockFactor = 1;
    }
    tiling->blockFactor = blockFactor;

    // one UB tile per core to reduce queue overhead
