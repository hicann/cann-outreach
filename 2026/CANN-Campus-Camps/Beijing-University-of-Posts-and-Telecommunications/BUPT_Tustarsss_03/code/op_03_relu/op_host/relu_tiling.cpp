...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    const int64_t usedCoreNum = coreNum < 8 ? coreNum : 8;
    const int64_t blockFactor = CeilDiv(totalNum, usedCoreNum);
    // Two queues, each double-buffered. Budget with float32, the larger dtype.
    constexpr int64_t queueBufferCount = 4;
    int64_t ubFactor = FloorAlign(static_cast<int64_t>(ubSize) /
                                  (TYPE_SIZE * queueBufferCount), 8L);
