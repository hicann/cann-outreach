...
constexpr int64_t TILING_BUFFER_NUM = 2;
...
static ge::graphStatus ReluTilingFunc(gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(totalNum <= 0, OP_LOGE(context, "input element count must be positive"),
    ...
    const int64_t blockBytes = static_cast<int64_t>(GetUbBlockSize(context));
    const int64_t alignNum = blockBytes / TYPE_SIZE;
    const int64_t queueCount = 2;
    const int64_t maxUbFactor = FloorAlign(
        static_cast<int64_t>(ubSize) / (TYPE_SIZE * TILING_BUFFER_NUM * queueCount), alignNum);
    ...
    tiling->ubFactor = static_cast<uint32_t>(blockFactor < maxUbFactor ? blockFactor : maxUbFactor);
