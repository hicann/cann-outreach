...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum) {
        ...
        this->length = this->blockLength / this->tileNum / BUFFER_NUM;
        ...
        pipe.InitBuffer(inQueueX, BUFFER_NUM, this->length * sizeof(DT_X));
        pipe.InitBuffer(inQueueY, BUFFER_NUM, this->length * sizeof(DT_X));
        pipe.InitBuffer(outQueueZ, BUFFER_NUM, this->length * sizeof(DT_X));
    }
...
private:
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        AscendC::DataCopy(xLocal, xGm[progress * this->length], this->length);
        AscendC::DataCopy(yLocal, yGm[progress * this->length], this->length);
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Mul(zLocal, xLocal, yLocal, this->length);
        // ---- 步骤9：结果入队，释放输入 ----
    ...
    __aicore__ inline void CopyOut(int32_t progress)
    {
        ...
        AscendC::DataCopy(zGm[progress * this->length], zLocal, this->length);
