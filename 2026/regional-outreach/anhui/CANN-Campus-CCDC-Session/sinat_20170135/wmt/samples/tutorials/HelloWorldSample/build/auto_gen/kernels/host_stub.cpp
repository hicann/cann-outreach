...
struct ascend_kernels {
    ...
    uint8_t aiv_buf[16000];
} __ascend_kernel_ascend910b3_kernels __attribute__ ((section (".ascend.kernel.ascend910b3.kernels"))) = {1,1,1,16000,16000,{0}};
...
void __register_kernels(void)
{
    ...
    ret = RegisterAscendBinary(
        (const char *)__ascend_kernel_ascend910b3_kernels.aiv_buf,
        __ascend_kernel_ascend910b3_kernels.aiv_file_len,
        1,
        &g_kernel_handle_aiv);
    if (ret != 0) {
