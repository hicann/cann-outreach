...
```
CANN软件包中提供了工程创建工具msopgen，AddCustom算子工程可通过AddCustom.json自动创建，具体请参考[Ascend C算子开发](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC)>工程化算子开发>创建算子工程 章节。
## 编译运行样例算子
...
### 2. 编译算子工程<a name="operatorcompile"></a>
  ...
  - 执行如下命令，切换到算子工程AddCustom目录。   
    ...
    ```bash
    cd ${git_clone_path}/samples/operator/ascendc/tutorials/AddCustomSample/FrameworkLaunch/AddCustom
    ```
