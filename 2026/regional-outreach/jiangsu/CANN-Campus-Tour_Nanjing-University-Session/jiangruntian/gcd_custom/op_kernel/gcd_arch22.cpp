...
/*!
 * \file gcd_arch22.cpp
 * \brief GCD 算子 kernel 入口（arch22 架构 - ascend910b）
 *
 * 模板参数：
 *   - D_T_X: float16（由 tiling_key 定义）
 *   - BUFFER_MODE: 缓冲模式（0=单缓冲, 1=双缓冲）
 */
#include "arch22/gcd.h"

template <typename D_T_X, int BUFFER_MODE>
__global__ __aicore__ void gcd(GM_ADDR self, GM_ADDR other, GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling)
{
