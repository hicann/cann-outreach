...
# ============================================================================
# Ascend C Kernel 直调运行脚本 - Gcd Float16
# 编译 → 生成测试数据 → 运行 → 精度验证
#
# 用法:
#   bash run.sh                        # 完整流程 (默认测试)
...
echo "=== [3/4] 生成测试数据 ==="
cd build
python3 ../scripts/gen_data.py --case all || die "gen_data.py 执行失败"
...
echo "=== 精度验证 ==="
# 验证 Case C (最大测试用例)
python3 ../scripts/verify_result.py output/output.bin output/C/golden.bin \
