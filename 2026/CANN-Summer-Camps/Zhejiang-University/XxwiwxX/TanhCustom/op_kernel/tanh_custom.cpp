...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        const uint32_t avgBlockLength = (totalLength + blockNum - 1) / blockNum;
        const uint32_t blockOffset = avgBlockLength * blockIdx;

        if (blockOffset >= totalLength) {
            this->blockLength = 0;
        } else {
            const uint32_t remainLength = totalLength - blockOffset;
            this->blockLength = remainLength < avgBlockLength ? remainLength : avgBlockLength;
        }

        const uint32_t splitNum = this->tileNum * BUFFER_NUM;
        this->tileLength = (this->blockLength + splitNum - 1) / splitNum;
        this->tileLength = this->tileLength == 0 ? 1 : this->tileLength;
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(numerator, xLocal, static_cast<DTYPE_X>(-1.0f), this->tileLength);
        AscendC::Exp(expNegX, numerator, this->tileLength);
        AscendC::Sub(numerator, expX, expNegX, this->tileLength);
        AscendC::Add(expX, expX, expNegX, this->tileLength);
        AscendC::Div(yLocal, numerator, expX, this->tileLength);
