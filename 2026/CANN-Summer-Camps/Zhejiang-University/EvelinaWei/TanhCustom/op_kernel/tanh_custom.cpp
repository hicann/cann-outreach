...
constexpr int32_t kBufferNum = 2; // tensor num for each queue
constexpr int32_t kTileNum = 8;
constexpr int32_t kBlockDim = 8;
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        this->blockLength = (totalLength + kBlockDim - 1) / kBlockDim;
        ...
        pipe.InitBuffer(inQueueX, kBufferNum, this->tileLength * sizeof(DTYPE_X));
        pipe.InitBuffer(outQueueY, kBufferNum, this->tileLength * sizeof(DTYPE_Y));
    ...
    {
        int32_t loopCount = this->tileNum * kBufferNum;
...
private:
    ...
    {
        int32_t tileIdx = progress / kBufferNum;
...
private:
    AscendC::TPipe pipe;
    AscendC::TQue<AscendC::QuePosition::VECIN, kBufferNum> inQueueX;
    AscendC::TQue<AscendC::QuePosition::VECOUT, kBufferNum> outQueueY;
...
extern "C" __global__ __aicore__ void tanh_custom(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    KernelTanh op;
    op.Init(x, y, static_cast<uint32_t>(tilingData.totalNum), kTileNum);
    op.Process();
}
