...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint64_t totalLength, uint32_t tileNum)
    {
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(expNeg, xLocal, (DTYPE_X)(-1), this->tileLength);
        AscendC::Exp(expNeg, expNeg, this->tileLength);

        AscendC::Sub(numerator, expPos, expNeg, this->tileLength);
        AscendC::Add(expPos, expPos, expNeg, this->tileLength);

        AscendC::Div(yLocal, numerator, expPos, this->tileLength);
