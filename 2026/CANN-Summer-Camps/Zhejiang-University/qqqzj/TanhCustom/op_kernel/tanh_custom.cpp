...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::LocalTensor<DTYPE_X> work = tmpBuf2.Get<DTYPE_X>();

        AscendC::Exp(expX, xLocal, this->tileLength);
        AscendC::Muls(work, xLocal, static_cast<DTYPE_X>(-1.0), this->tileLength);
        AscendC::Exp(expNegX, work, this->tileLength);
        AscendC::Sub(work, expX, expNegX, this->tileLength);
        AscendC::Add(expX, expX, expNegX, this->tileLength);
        AscendC::Div(yLocal, work, expX, this->tileLength);
