...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        ASSERT(this->blockLength != 0 && "block length can not be zero!");
        ASSERT(this->tileLength != 0 && "tile length can not be zero!");

        xGm.SetGlobalBuffer((__gm__ DTYPE_X*)x + this->blockLength * blockIdx, this->blockLength);
        yGm.SetGlobalBuffer((__gm__ DTYPE_Y*)y + this->blockLength * blockIdx, this->blockLength);
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        AscendC::Muls(tmpLocal0, xLocal, static_cast<DTYPE_X>(-1.0f), this->tileLength);
        AscendC::Exp(tmpLocal1, xLocal, this->tileLength);
        AscendC::Exp(yLocal, tmpLocal0, this->tileLength);
        AscendC::Sub(tmpLocal0, tmpLocal1, yLocal, this->tileLength);
        AscendC::Add(tmpLocal2, tmpLocal1, yLocal, this->tileLength);
        AscendC::Div(yLocal, tmpLocal0, tmpLocal2, this->tileLength);
