...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        // tmp2 = -x
        AscendC::Muls(tmp2, tmp0, -1.0f, this->tileLength);
        AscendC::PipeBarrier<PIPE_V>();

        // tmp2 = exp(-x)
        AscendC::Exp(tmp2, tmp2, this->tileLength);
        AscendC::PipeBarrier<PIPE_V>();

	// tmp0 = exp(x) - exp(-x)
        AscendC::Sub(tmp0, tmp1, tmp2, this->tileLength);
        AscendC::PipeBarrier<PIPE_V>();

        // tmp1 = exp(x) + exp(-x)
        AscendC::Add(tmp1, tmp1, tmp2, this->tileLength);
        AscendC::PipeBarrier<PIPE_V>();

        // tmp0 = tanh(x)
        AscendC::Div(tmp0, tmp0, tmp1, this->tileLength);
        AscendC::PipeBarrier<PIPE_V>();
