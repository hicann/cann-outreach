...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    tilingData.totalLength = context->GetInputShape(0)->GetStorageShape().GetShapeSize();
    tilingData.tileNum = TILE_NUM;

    context->SetBlockDim(BLOCK_DIM);
    std::memcpy(context->GetRawTilingData()->GetData(), &tilingData, sizeof(TanhCustomTilingData));
