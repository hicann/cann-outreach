...
public:
                                ...
                                uint32_t totalLength, uint32_t tileNum)
    {
        ...
        xGm_.SetGlobalBuffer(
            (__gm__ TX *)xAddr + blockOffset_,
            blockLength_);

        yGm_.SetGlobalBuffer(
            (__gm__ TY *)yAddr + blockOffset_,
            blockLength_);

        zGm_.SetGlobalBuffer(
            (__gm__ TZ *)zAddr + blockOffset_,
            blockLength_);

        pipe_.InitBuffer(
            xQueue_,
            DOUBLE_BUFFER,
            tileLength_ * sizeof(TX));

        pipe_.InitBuffer(
            yQueue_,
            DOUBLE_BUFFER,
            tileLength_ * sizeof(TY));

        pipe_.InitBuffer(
            zQueue_,
            DOUBLE_BUFFER,
            tileLength_ * sizeof(TZ));
    }

    __aicore__ inline void Process()
    {
        for (uint32_t tileIndex = 0U;
             tileIndex < tileCount_;
             ++tileIndex) {
...
private:
    ...
    {
        AscendC::LocalTensor<TX> xLocal =
            xQueue_.AllocTensor<TX>();

        AscendC::LocalTensor<TY> yLocal =
            yQueue_.AllocTensor<TY>();
    ...
    __aicore__ inline void Compute(uint32_t length)
    {
        ...
        AscendC::LocalTensor<TZ> zLocal =
            zQueue_.AllocTensor<TZ>();
...
private:
    ...
    AscendC::TQue<
        ...
        DOUBLE_BUFFER> xQueue_;

    AscendC::TQue<
        AscendC::TPosition::VECIN,
        DOUBLE_BUFFER> yQueue_;

    AscendC::TQue<
        AscendC::TPosition::VECOUT,
        DOUBLE_BUFFER> zQueue_;
