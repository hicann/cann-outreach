...
class KernelSub {
public:
    __aicore__ inline KernelSub() : blockLength(0), tileNum(0), tileLength(0) {}
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        xGm.SetGlobalBuffer((__gm__ dtypeX *)x + (totalLength / AscendC::GetBlockNum()) * AscendC::GetBlockIdx(), this->blockLength);
        yGm.SetGlobalBuffer((__gm__ dtypeY *)y + (totalLength / AscendC::GetBlockNum()) * AscendC::GetBlockIdx(), this->blockLength);
        zGm.SetGlobalBuffer((__gm__ dtypeZ *)z + (totalLength / AscendC::GetBlockNum()) * AscendC::GetBlockIdx(), this->blockLength);
