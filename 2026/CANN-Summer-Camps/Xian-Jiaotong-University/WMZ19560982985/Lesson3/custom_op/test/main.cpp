    ...
    void *workspaceAddr = nullptr)
{
    for (uint32_t i = 0; i < tensors.size(); i++) {
        ...
        if (tensors[i] != nullptr) {
            aclDestroyTensor(
                reinterpret_cast<aclTensor *>(tensors[i]));
        }
...
int main(int argc, char **argv)
{
    ...
    std::vector<float> inputXHostData(
        inputXShape[0] * inputXShape[1]);

    std::vector<float> inputYHostData(
        inputYShape[0] * inputYShape[1]);

    std::vector<float> outputZHostData(
        outputZShape[0] * outputZShape[1]);


    for (int i = 0;
         i < inputXShape[0] * inputXShape[1];
         ++i) {
    ...
    // 保存真实申请成功的资源
    std::vector<void *> tensors(3, nullptr);
    std::vector<void *> deviceAddrs(3, nullptr);
    ...
    ret = aclnnAddCustomTemplateGetWorkspaceSize(
        inputX,
        inputY,
        outputZ,
        &workspaceSize,
    ...
    CHECK_RET(ret == ACL_SUCCESS,
        LOG_PRINT(
            "aclnnAddCustomTemplateGetWorkspaceSize failed. ERROR: %d\n",
            ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS,
        LOG_PRINT(
            "copy result from device to host failed. ERROR: %d\n",
            ret);
