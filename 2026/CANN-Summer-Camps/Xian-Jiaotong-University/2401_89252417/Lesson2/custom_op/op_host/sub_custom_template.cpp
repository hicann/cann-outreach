...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
  ...
  // Shape is (8, 2048). Eight AI Cores process one row each. Each core
  // executes two tiles so the two queue buffers can form a double buffer.
  tiling->totalLength = totalLength;
  tiling->tileNum = 2;
  context->SetBlockDim(8);
  const auto dataType = context->GetInputDesc(0)->GetDataType();
  context->SetTilingKey(dataType == ge::DT_FLOAT ? 1 : 0);
