...
if ./truncate_mod_op_host_ut; then
...
# 运行 Kernel UT（数据目录通过 argv[1] 传入）
cd "${BUILD_DIR}/op_kernel"
if ./truncate_mod_op_kernel_ut "${DATA_DIR}"; then
...
if [ -d "${BUILD_DIR}/../../../op_api" ]; then
    cd "${BUILD_DIR}/op_api"
    if ./truncate_mod_op_api_ut; then
        PASSED_TESTS+=("op_api")
    else
        FAILED_TESTS+=("op_api")
    fi
