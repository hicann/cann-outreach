...
def calculate_mare(actual, golden):
    """计算最大相对误差(MARE)，忽略 golden/actual 为非有限（NaN/Inf）的位置（特殊值由 isclose_ratio 按位判定）"""
    valid = np.isfinite(golden) & np.isfinite(actual)
    if not np.any(valid):
        return 0.0
    relative_errors = np.abs(actual[valid] - golden[valid]) / np.maximum(np.abs(golden[valid]), 1e-6)
    return np.max(relative_errors)


def calculate_mere(actual, golden):
    """计算平均相对误差(MERE)，忽略 golden/actual 为非有限的位置"""
    valid = np.isfinite(golden) & np.isfinite(actual)
    if not np.any(valid):
        return 0.0
    relative_errors = np.abs(actual[valid] - golden[valid]) / np.maximum(np.abs(golden[valid]), 1e-6)
...
def isclose_ratio(actual, golden, threshold):
    """元素级容差比对（NaN==NaN、同号 Inf 视为一致），返回匹配比例"""
    # 拆分有限与特殊值：有限元素用 isclose；NaN/Inf 按位语义
    finite_golden = np.isfinite(golden)
    matched = np.zeros_like(golden, dtype=bool)
    if np.any(finite_golden):
        matched[finite_golden] = np.isclose(actual[finite_golden], golden[finite_golden],
                                            rtol=threshold, atol=threshold)
    special = ~finite_golden
    if np.any(special):
        # NaN == NaN；+Inf == +Inf；-Inf == -Inf（不匹配有限/异号）
        both_nan = np.isnan(actual[special]) & np.isnan(golden[special])
        both_pinf = (actual[special] == np.inf) & (golden[special] == np.inf)
        both_ninf = (actual[special] == -np.inf) & (golden[special] == -np.inf)
        matched[special] = both_nan | both_pinf | both_ninf
...
def compare_data_float(golden_file_lists, output_file_lists, d_type):
    ...
    def _read_bin(path):
        ...
        if d_type == "fp8_e4m3fn":
            ...
            # ml_dtypes bfloat16 无法与 np.float64 标量在 isclose 中提升（DTypePromotionError），
            # 先无损升到 float32（bf16→fp32 保真）再参与全部比对
    ...
    if len(golden_file_lists) == 1 and len(output_file_lists) > 1:
        ...
        ratio = isclose_ratio(tmp_out, tmp_gold, threshold)

        mere_pass = mere < threshold
        mare_pass = mare < mare_threshold
        ratio_pass = ratio >= 0.99  # 特殊值（NaN/Inf）按位匹配比例
        is_pass = mere_pass and mare_pass and ratio_pass

        if is_pass:
            print(f"PASSED! MERE={mere:.6f}, MARE={mare:.6f}, match_ratio={ratio:.4f}")
        else:
            print(f"FAILED! MERE={mere:.6f} (threshold={threshold:.6f}), "
                  f"MARE={mare:.6f} (threshold={mare_threshold:.6f}), match_ratio={ratio:.4f}")
...
def get_file_lists(dtype):
    """按 dtype 前缀过滤 golden/output 文件（多 dtype 共存时互不干扰）"""
    golden_file_lists = sorted(glob.glob(curr_dir + f"/{dtype}_*golden*.bin"))
    output_file_lists = sorted(glob.glob(curr_dir + f"/{dtype}_*output*.bin"))
    return golden_file_lists, output_file_lists


def detect_dtypes():
    """从文件名检测所有存在的 dtype"""
    files = glob.glob(curr_dir + "/*golden*.bin")
    dtypes = []
    for dt in ["float16", "float32", "float64", "bfloat16", "fp8_e4m3fn", "fp8_e5m2", "hifloat32",
               "int8", "int16", "int32", "int64", "uint8", "uint16", "uint32", "uint64", "bool"]:
        if any(os.path.basename(f).startswith(dt + "_") for f in files):
            dtypes.append(dt)
    return dtypes
...
if __name__ == '__main__':
    # 自动检测所有 dtype 分组分别比对；也可通过命令行参数指定单一 dtype
    if len(sys.argv) >= 2:
        d_types = [sys.argv[1]]
        print(f"指定 dtype: {d_types[0]}")
    else:
        d_types = detect_dtypes()
        print(f"检测到 dtype: {d_types}")

    all_ok = True
    for d_type in d_types:
        print(f"===== 比对 dtype={d_type} =====")
        ret = process(d_type)
        if not ret:
            all_ok = False
