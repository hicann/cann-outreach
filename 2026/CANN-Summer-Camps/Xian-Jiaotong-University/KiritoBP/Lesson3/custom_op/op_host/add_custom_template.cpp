...
namespace optiling {
// 动态 Tiling 策略: 根据 totalLength 自动选择最优 tileNum 和 BlockDim
...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    // 动态选择 BlockDim: 最大化并行度，每核处理 TARGET_TILE_LENGTH 元素
    uint32_t blockDim = MAX_BLOCK_DIM;
    while (blockDim > 1 && (uint32_t)(totalLength / blockDim) < TARGET_TILE_LENGTH) {
        blockDim >>= 1;
    }
    context->SetBlockDim(blockDim);

    uint32_t blockLength = (uint32_t)(totalLength / blockDim);

    // 动态选择 tileNum: 减少 tile 数量以降低循环/调度开销
    // 原基准 tileNum=360, 优化后 tileNum=4 (每核一大块)
