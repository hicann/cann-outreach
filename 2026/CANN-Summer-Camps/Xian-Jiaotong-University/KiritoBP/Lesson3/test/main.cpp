...
int main(int argc, char **argv)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, aclDestroyTensor(inputX); aclrtFree(inputXDeviceAddr); return FAILED);
    // Create outputZ aclTensor
    ret = CreateAclTensor(outputZHostData, outputZShape, &outputZDeviceAddr, aclDataType::ACL_FLOAT, &outputZ);
    CHECK_RET(ret == ACL_SUCCESS, aclDestroyTensor(inputX); aclDestroyTensor(inputY);
              aclrtFree(inputXDeviceAddr); aclrtFree(inputYDeviceAddr); return FAILED);
    // Capture updated pointers after all CreateAclTensor calls succeed
