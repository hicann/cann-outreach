...
constexpr int64_t MIN_DOUBLE_BUFFER = 512;
...
static ge::graphStatus TruncateModTilingFunc(gert::TilingContext* context)
{
    ...
    td->blockFactor = CeilAlign(CeilDiv(totalNum, coreNum), ubBlockSize);
    int64_t usedCoreNum = CeilDiv(totalNum, td->blockFactor);

    int64_t useDoubleBuf = (totalNum > MIN_DOUBLE_BUFFER) ? 1 : 0;
