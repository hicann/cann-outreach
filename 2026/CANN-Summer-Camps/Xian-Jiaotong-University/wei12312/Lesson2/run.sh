...
: "${TOOLKIT_HOME:?Error: CANN toolkit path is not set after loading the environment.}"
...
# 清理旧的构建目录以确保干净编译
rm -rf -- "${SCRIPT_DIR}/custom_op/build_out"
echo ">>> Running build.sh..."
bash "${SCRIPT_DIR}/custom_op/build.sh"

echo "=========================================="
echo " 3. Installing Custom Operator"
echo "=========================================="
RUN_FILE="$(find "${SCRIPT_DIR}/custom_op/build_out" -maxdepth 1 -type f -name 'custom_opp*.run' -print -quit)"
