...
int main(int argc, char **argv)
{
    ...
    auto destroyCurrentResources = [&](void *workspaceAddr = nullptr) {
        const std::vector<void *> tensors = {inputX, inputY, outputZ};
        const std::vector<void *> deviceAddrs = {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr};
    ...
    CHECK_RET(ret == ACL_SUCCESS, destroyCurrentResources(); return FAILED);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddCustomTemplateGetWorkspaceSize failed. ERROR: %d\n", ret);
              destroyCurrentResources(); return FAILED);

    void *workspaceAddr = nullptr;
    if (workspaceSize > 0) {
        ret = aclrtMalloc(&workspaceAddr, workspaceSize, ACL_MEM_MALLOC_HUGE_FIRST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret);
                  destroyCurrentResources(workspaceAddr); return FAILED);
    }
    ...
    destroyCurrentResources(workspaceAddr);
