...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        xGm.SetGlobalBuffer((__gm__ dtypeX *)x + blockIdx * this->blockLength, this->blockLength);
        yGm.SetGlobalBuffer((__gm__ dtypeY *)y + blockIdx * this->blockLength, this->blockLength);
        zGm.SetGlobalBuffer((__gm__ dtypeZ *)z + blockIdx * this->blockLength, this->blockLength);
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t i = 2; i < this->tileNum; i++) {
            CopyIn(i);
            Compute(i-1);
            CopyOut(i-2);
        }
...
private:
    __aicore__ inline void CopyIn(uint32_t tileIdx)
    {
        ...
        AscendC::DataCopy(xLocal, xGm[tileIdx * this->tileLength], this->tileLength);
        AscendC::DataCopy(yLocal, yGm[tileIdx * this->tileLength], this->tileLength);
    ...
    __aicore__ inline void CopyOut(uint32_t tileIdx)
    {
        ...
        AscendC::DataCopy(zGm[tileIdx * this->tileLength], zLocal, this->tileLength);
