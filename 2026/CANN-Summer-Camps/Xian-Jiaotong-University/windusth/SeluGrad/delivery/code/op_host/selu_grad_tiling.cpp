...
static ge::graphStatus SeluGradTilingFunc(gert::TilingContext* context)
{
    ...
    int64_t calcBytes = inputDesc->GetDataType() == ge::DT_FLOAT ? FLOAT_SIZE : FLOAT_SIZE * 4;
    int64_t usableUb = static_cast<int64_t>(ubSize) - SELECT_TMP_RESERVED;
    OP_CHECK_IF(usableUb <= 0, OP_LOGE(context, "UB is too small for SeluGrad"), return ge::GRAPH_FAILED);
    ...
    switch (inputDesc->GetDataType()) {
        case ge::DT_FLOAT16:
            tilingKey = GET_TPL_TILING_KEY(SELUGRAD_TPL_SCH_MODE_0);
            break;
        case ge::DT_FLOAT:
            tilingKey = GET_TPL_TILING_KEY(SELUGRAD_TPL_SCH_MODE_1);
            break;
        case ge::DT_BF16:
            tilingKey = GET_TPL_TILING_KEY(SELUGRAD_TPL_SCH_MODE_2);
            break;
        default:
            OP_LOGE(context, "Unsupported dtype for SeluGrad");
            return ge::GRAPH_FAILED;
    }
