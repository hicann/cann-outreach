...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint64_t totalLength, uint32_t tileNum)
    {
        ...
        // Split by 32B blocks so GM offsets and DataCopy lengths stay aligned.
        ...
        // BUFFER_NUM=2 lets TQue overlap copy and vector add in the steady state.
        this->loopCount = static_cast<uint64_t>(tileNum) * BUFFER_NUM;
    ...
    {
        for (uint64_t i = 0; i < this->loopCount; ++i) {
            const uint64_t offset = i * this->tileLength;
            if (offset >= this->blockLength) {
                return;
            }
            const uint64_t length = (offset + this->tileLength <= this->blockLength) ?
