...
int main()
{
    ...
    void* gradientsDeviceAddr = nullptr;
    aclTensor* outputs = nullptr;
    void* outputsDeviceAddr = nullptr;
    std::vector<int64_t> gradientsShape = {5};
    std::vector<float> gradientsHostData(5, 1);
    ret = CreateAclTensor(gradientsHostData, gradientsShape, &gradientsDeviceAddr, aclDataType::ACL_FLOAT16, &gradients);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> outputsShape = {5};
    std::vector<float> outputsHostData(5, 1);
    ret = CreateAclTensor(outputsHostData, outputsShape, &outputsDeviceAddr, aclDataType::ACL_FLOAT16, &outputs);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    // 调用 aclnnSeluGradGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnSeluGradGetWorkspaceSize(gradients, outputs, y, &workspaceSize, &executor);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnSeluGradGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnSeluGrad 第二段接口
    ret = aclnnSeluGrad(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnSeluGrad failed. ERROR: %d\n", ret); return ret);
    ...
    aclrtFree(gradientsDeviceAddr);
    aclDestroyTensor(outputs);
    aclrtFree(outputsDeviceAddr);
