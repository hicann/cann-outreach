...
// Double buffering overlaps GM transfers with vector computation.  With two
// logical tiles per core, each physical tile contains 5,760 float elements for
// the benchmark shape, keeping the three queues within UB capacity.
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        this->blockLength = baseBlockLength + (blockIdx < remainder ? 1 : 0);
        const uint32_t blockOffset = blockIdx * baseBlockLength + (blockIdx < remainder ? blockIdx : remainder);
        ...
        const uint32_t xBufferSize = (this->tileLength * sizeof(dtypeX) + 31) / 32 * 32;
        const uint32_t yBufferSize = (this->tileLength * sizeof(dtypeY) + 31) / 32 * 32;
        const uint32_t zBufferSize = (this->tileLength * sizeof(dtypeZ) + 31) / 32 * 32;
