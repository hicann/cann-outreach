...
int main()
{
    ...
    // 调用 aclnnTruncateModGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnTruncateModGetWorkspaceSize(x1, x2, y, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTruncateModGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnTruncateMod 第二段接口
    ret = aclnnTruncateMod(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTruncateMod failed. ERROR: %d\n", ret); return ret);
