...
constexpr int TRUNCATE_MOD_MAX_TENSOR_DIM = 8;
...
{
    // 检查 self 的数据类型是否在 TruncateMod 支持列表内
    OP_CHECK_DTYPE_NOT_SUPPORT(self, DTYPE_SUPPORT_LIST, return false);

    // 检查 other 的数据类型是否在 TruncateMod 支持列表内
...
{
    OP_CHECK_MAX_DIM(self, TRUNCATE_MOD_MAX_TENSOR_DIM, return false);
    OP_CHECK_MAX_DIM(other, TRUNCATE_MOD_MAX_TENSOR_DIM, return false);
...
aclnnStatus aclnnTruncateModTensorGetWorkspaceSize(const aclTensor* self, const aclTensor* other, aclTensor* out,
                                                   uint64_t* workspaceSize, aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnTruncateModTensor, DFX_IN(self, other), DFX_OUT(out));
    ...
    // TruncateMod算子的空tensor在kernel中支持，对标竞品根据算子实际情况补充
    ...
    // TruncateMod算子需要对self和other两个输入做隐式数据类型转换，根据具体算子语义按需调用
    ...
    // 进行TruncateMod计算
    const aclTensor* truncateModOut = l0op::TruncateMod(selfCasted, otherCasted, uniqueExecutor.get());
    CHECK_RET(truncateModOut != nullptr, ACLNN_ERR_INNER_NULLPTR);

    // 固定写法，将计算结果转换成输出out的数据类型
    auto castOut = l0op::Cast(truncateModOut, out->GetDataType(), uniqueExecutor.get());
...
aclnnStatus aclnnInplaceTruncateModTensorGetWorkspaceSize(const aclTensor* selfRef, const aclTensor* other,
                                                          uint64_t* workspaceSize, aclOpExecutor** executor)
{
    auto out = const_cast<aclTensor*>(selfRef);
    return aclnnTruncateModTensorGetWorkspaceSize(selfRef, other, out, workspaceSize, executor);
}

aclnnStatus aclnnTruncateModTensor(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnTruncateModTensor);
    // 固定写法，调用框架能力，完成计算
    return CommonOpExecutorRun(workspace, workspaceSize, executor, stream);
}

aclnnStatus aclnnInplaceTruncateModTensor(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                          aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnInplaceTruncateModTensor);
    // 固定写法，调用框架能力，完成计算
    return CommonOpExecutorRun(workspace, workspaceSize, executor, stream);
}
