...
int main(int argc, char **argv)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("CreateAclTensor inputX failed. ERROR: %d\n", ret);
              DestroyResources({inputX, inputY, outputZ}, {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}, 
                               stream, deviceId); return FAILED);
    
    // Create inputY aclTensor
    ret = CreateAclTensor(inputYHostData, inputYShape, &inputYDeviceAddr, aclDataType::ACL_FLOAT16, &inputY);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("CreateAclTensor inputY failed. ERROR: %d\n", ret);
              DestroyResources({inputX, inputY, outputZ}, {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}, 
                               stream, deviceId); return FAILED);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("CreateAclTensor outputZ failed. ERROR: %d\n", ret);
              DestroyResources({inputX, inputY, outputZ}, {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}, 
                               stream, deviceId); return FAILED);
    ...
    if (workspaceSize > 0) {
        ...
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret);
                  DestroyResources({inputX, inputY, outputZ}, {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}, 
                                   stream, deviceId, workspaceAddr); return FAILED);
    }
