...
2. **浮点输入**：浮点路径同样对除法结果执行向零截断。例如 7.0 除以 3.0 的结果为 2.0，-7.0 除以 3.0 的结果为 -2.0。
### 3.3 输入输出参数定义
...
3. 两维相等时输出该维；两维不等时输出其中不为 1 的维度。该规则也正确覆盖 0 维长度与 1 的广播。
...
| Tiling 策略    | `truncate_div_tiling.cpp`     | 平台信息查询、合法性校验、分核、UB tile 量化、stride 表生成                 |
| Kernel 计算    | `truncate_div.cpp`            | 分类型 Kernel 模板实现，按 TilingData 模式字段分派计算路径                  |
...
4. **Shape 处理**：将输入 Shape 右对齐填充至 8 维，推导输出 Shape 与总元素数，分类广播模式（相同shape/x1标量/x2标量/单边广播/双向广播）
...
9. **Tiling 数据写入**：填充 TilingData 结构，设置 BlockDim；当前实现使用单一 TilingKey，由 Kernel 根据 TilingData 中的模式字段分派
### 5.4 TilingData 结构
...
### 5.5 TilingKey
当前实现固定使用 TilingKey 0，仅生成一个 Kernel 变体。数据类型和广播模式通过 TilingData 下发，并在 Kernel 入口及 CopyIn 阶段分派，减少模板组合数量与编译产物体积。

## 6 Kernel侧设计
### 6.1 Kernel 总体结构
Kernel 入口按 TilingData 中的 `dtypeMode` 分派至三类实现路径：
| 路径         | 适用场景                        | 计算流程                       |
| :----------- | :------------------------------ | :----------------------------- |
...
6. 结果回写 GM
### 6.5 Kernel 入口分派
Kernel 入口函数接收 TilingData，根据 `dtypeMode` 分派到浮点、INT8/UINT8 或 INT32 实现；各实现再根据 `broadcastMode` 选择连续搬运、标量填充或 stride 索引路径。
...
| 单边/双向广播    | stride 索引搬运    | stride 索引搬运    | Host 预计算 stride，Kernel 映射输入偏移 |
### 8.4 对齐优化
...
3. **广播快速路径**：相同 shape 使用连续 DataCopy，标量输入使用 GetValue+Duplicate，避免通用 stride 映射开销
4. **中间类型优化**：整数路径统一用 FP32 中间计算对齐 TBE 精度；INT8/UINT8 经 half 中转节省 UB 占用
5. **UB 紧凑复用**：中间计算缓冲通过单个 TBuf 切分复用，减少 UB 浪费，提升单 tile 元素数
6. **对齐搬运**：256 字节对齐提升 DataCopy burst 效率，最大化带宽利用率
...
| 除法后截断的边界舍入 | 始终使用 `Div→Trunc` 参考路径，避免倒数乘法改变临界值 |
