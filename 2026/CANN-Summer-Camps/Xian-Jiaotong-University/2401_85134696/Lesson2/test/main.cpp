...
// 修改 1：显式计算 stride，检查 aclCreateTensor 返回值，增加诊断打印
template <typename T>
                    ...
                    aclDataType dataType, aclTensor **tensor)
{
    ...
    // 显式计算 stride（row-major 连续布局），不再传 nullptr，避免 aclCreateTensor 返回空指针
    ...
    *tensor = aclCreateTensor(shape.data(), shape.size(), dataType, stride.data(), 0, aclFormat::ACL_FORMAT_ND,
    ...
    // 检查返回值，避免静默 nullptr 导致后续 161001
    if (*tensor == nullptr) {
        LOG_PRINT("aclCreateTensor returned nullptr! dimNum=%zu, dtype=%d, devAddr=%p\n", shape.size(),
                  static_cast<int>(dataType), *deviceAddr);
        return FAILED;
    }
    LOG_PRINT("tensor created OK: %p (devAddr=%p)\n", *tensor, *deviceAddr);
    return SUCCESS;
}
...
int main(int argc, char **argv)
{
    ...
    // 立即用最新指针刷新清理列表：即使本次创建失败，已分配的 device 内存也能被释放，避免泄漏
    tensors[0] = inputX;
    ...
    // 创建完成后再次确认清理列表指向最新指针，保证成功/失败路径都能正确释放资源
    tensors = {inputX, inputY, outputZ};
    deviceAddrs = {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr};

    // 3. Call the API of the custom operator library
    uint64_t workspaceSize = 0;
    aclOpExecutor *executor;
    // 修改 3：诊断打印，确认传入算子的 tensor 指针均非空
    LOG_PRINT("[DBG] inputX=%p inputY=%p outputZ=%p\n", inputX, inputY, outputZ);
    ...
    if (ret != ACL_SUCCESS) {
        ...
        LOG_PRINT("[RECENT_ERR] %s\n", aclGetRecentErrMsg());   // ← 框架记录的真实原因
