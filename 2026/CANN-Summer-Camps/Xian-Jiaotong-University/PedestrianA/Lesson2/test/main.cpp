                    ...
                    aclDataType dataType, aclTensor **tensor)
{
    ...
    if (ret != ACL_SUCCESS) {
        ...
        aclrtFree(*deviceAddr); // 新增：释放已分配的内存
        *deviceAddr = nullptr;  // 新增：置空指针防止二次释放
        return FAILED;
    }
...
int main(int argc, char **argv)
{
   ...
   // Create inputX aclTensor
    ...
    CHECK_RET(ret == ACL_SUCCESS, DestroyResources({inputX, inputY, outputZ}, {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}, stream, deviceId); return FAILED);
