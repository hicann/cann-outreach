...
constexpr int32_t BUFFER_NUM = 2;  // double buffering for DMA/compute overlap
constexpr int32_t ALIGN_SIZE = 16; // vector-friendly alignment
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        // tileLength based purely on blockLength/tileNum; BUFFER_NUM controls queue depth, not tile split
    ...
    __aicore__ inline void Process()
    {
        ...
        // Prefetch the first tile to prime the pipeline
        CopyIn(0);

        for (int32_t i = 0; i < loopCount - 1; i++) {
            CopyIn(i + 1);   // start DMA for next tile ¡ª overlaps with current compute/copyout
            Compute(i);      // compute current tile on vector unit
            CopyOut(i);      // DMA out current result ¡ª overlaps with next CopyIn
        }

        // Final tile: no next tile to prefetch
        Compute(loopCount - 1);
        CopyOut(loopCount - 1);
    }
