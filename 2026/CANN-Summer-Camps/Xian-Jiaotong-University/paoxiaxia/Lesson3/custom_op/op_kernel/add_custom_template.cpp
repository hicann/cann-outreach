...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        const uint32_t blockOffset = blockIdx * baseLength + (blockIdx < remainder ? blockIdx : remainder);
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t i = 0; i < loopCount; ++i) {
            ...
            const uint32_t currentLength = remain < this->tileLength ? remain : this->tileLength;
