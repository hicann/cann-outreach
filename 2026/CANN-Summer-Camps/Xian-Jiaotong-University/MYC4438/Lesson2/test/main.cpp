...
<<<<<<< HEAD
    CHECK_RET(ret == ACL_SUCCESS, std::vector<void *> tensors = {inputX, nullptr, nullptr}; std::vector<void *> deviceAddrs = {inputXDeviceAddr, nullptr, nullptr}; DestroyResources(tensors, deviceAddrs, stream, deviceId); return FAILED);
    // Create inputY aclTensor
    ret = CreateAclTensor(inputYHostData, inputYShape, &inputYDeviceAddr, aclDataType::ACL_FLOAT16, &inputY);
    CHECK_RET(ret == ACL_SUCCESS, std::vector<void *> tensors = {inputX, inputY, nullptr}; std::vector<void *> deviceAddrs = {inputXDeviceAddr, inputYDeviceAddr, nullptr}; DestroyResources(tensors, deviceAddrs, stream, deviceId); return FAILED);
    ...
    CHECK_RET(ret == ACL_SUCCESS, std::vector<void *> tensors = {inputX, inputY, outputZ}; std::vector<void *> deviceAddrs = {inputXDeviceAddr, inputYDeviceAddr, outputZDeviceAddr}; DestroyResources(tensors, deviceAddrs, stream, deviceId); return FAILED);
    
    // 所有tensor创建完成后，初始化资源列表
