...
constexpr int32_t BUFFER_NUM = 2;  // tensor num for each queue (double buffering)
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t totalLength, uint32_t tileNum)
    {
        ...
        this->tileNum = (tileNum > 0) ? tileNum : 1;
    ...
    {
        // Double buffering pipeline: prefetch first tile
        CopyIn(0);

        // Pipeline loop: load next tile while computing current tile
