...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
  ...
  constexpr uint32_t blockDim = 40;
  constexpr uint32_t tileNum = 4;
  constexpr uint32_t bufferNum = 1;
  constexpr uint32_t alignment = blockDim * tileNum * bufferNum;
  int64_t totalLength = context->GetInputShape(0)->GetOriginShape().GetShapeSize();
  if (totalLength < 0 || totalLength > std::numeric_limits<uint32_t>::max()) {
