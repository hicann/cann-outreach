...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    // 设置 tiling 数据：总元素数 -> 每核均分 -> UB 内单次处理块
    ...
    // 每核均分（向上取整），最后一个核按剩余量处理
    tiling->blockFactor = CeilDiv(totalNum, coreNum);

    // UB 单次处理块：封顶 4096，且满足 Compare API 的 256 字节对齐要求
    // （对齐元素数 = 256B / 元素宽度：fp32 为 64 的倍数，fp16 为 128 的倍数）
    auto inputDesc = context->GetInputDesc(0);
    int64_t typeSize = (inputDesc != nullptr && inputDesc->GetDataType() == ge::DT_FLOAT16) ? 2 : 4;
    int64_t alignElem = 256 / typeSize;
    int64_t ubFactor = tiling->blockFactor > 4096 ? 4096 : tiling->blockFactor;
    ubFactor = ubFactor / alignElem * alignElem;
    if (ubFactor == 0) {
        ubFactor = alignElem;
    }
    ...
    // 实际需要的核数（不超过平台核数）
    context->SetBlockDim(CeilDiv(totalNum, tiling->blockFactor));
