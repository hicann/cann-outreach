...
namespace optiling {
    // 性能优化：tileNum=1 使单次搬运块最大（tileLength = blockLength / 2 = 1024），
    // 每核仅循环 2 次，减少 DMA 搬运启动开销；双缓冲流水仍然生效
    const uint32_t TILE_NUM = 1;
    static ge::graphStatus TilingFunc(gert::TilingContext *context) {
        // 获取算子输入数组信息，选择TilingKey模板参数（按输入dtype实例化kernel）
