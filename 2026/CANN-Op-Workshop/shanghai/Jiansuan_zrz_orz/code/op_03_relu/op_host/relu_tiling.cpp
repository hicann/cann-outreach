...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(dtype != ge::DT_FLOAT && dtype != ge::DT_FLOAT16,
        OP_LOGE(context, "Only float32 and float16 are supported"), return ge::GRAPH_FAILED);
    ...
    const int64_t typeSize = ge::GetSizeByDataType(dtype);
    const int64_t alignNum = GetUbBlockSize(context) / typeSize;
    OP_CHECK_IF(alignNum <= 0, OP_LOGE(context, "Invalid UB block size"), return ge::GRAPH_FAILED);

    // 每核约4KB起分，减少小数据启动过多核的开销；核边界按UB块对齐。
    const int64_t minCoreNum = MIN_SPLIT_THRESHOLD * TYPE_SIZE / typeSize;
    int64_t blockDim = std::min(coreNum, std::max<int64_t>(1, CeilDiv(totalNum, minCoreNum)));
    const int64_t blockFactor = std::max(alignNum, CeilAlign(CeilDiv(totalNum, blockDim), alignNum));
    blockDim = std::max<int64_t>(1, CeilDiv(totalNum, blockFactor));

    // 一轮能放下时只分配输入/输出各一份；多轮时各分配两份以启用DoubleBuffer。
    const int64_t singleBufferCapacity = FloorAlign(static_cast<int64_t>(ubSize) / (2 * typeSize), alignNum);
    const int64_t ubFactor = blockFactor <= singleBufferCapacity ? blockFactor :
        FloorAlign(static_cast<int64_t>(ubSize) / (4 * typeSize), alignNum);
