...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    int64_t totalNum = inputShape->GetShape().GetShapeSize();
    ...
    int64_t alignNum = 32 / typeSize;
    if (alignNum < 1) {
        alignNum = 1;
    }
    blockFactor = CeilAlign(blockFactor, alignNum);

    int64_t ubFactor = std::min(blockFactor, int64_t(2048));
    ubFactor = CeilAlign(ubFactor, alignNum);
    if (ubFactor == 0) {
        ubFactor = alignNum;
    }
