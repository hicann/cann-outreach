...
#include "sub_tiling.h"
#include "tiling_key_sub.h"
constexpr int32_t BUFFER_NUM = 2; // tensor num for each queue
constexpr int32_t TILE_NUM = 8;   // 片内分块数
template <class DT_X>
class KernelSub {
public:
...
template <typename DT_X>
 __global__ __aicore__ void sub(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling) {
    REGISTER_TILING_DEFAULT(SubTilingData);
    GET_TILING_DATA_WITH_STRUCT(SubTilingData, tiling_data, tiling);
    KernelSub<DT_X> op;
